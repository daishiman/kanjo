...
confirmation_evidence: {"evaluator": "system-spec-harness:assign-system-spec-completeness-evaluator", "evidence_ref": "system-spec/archive/2026-09-10-retire-tax-receipt-and-clarify-freee-only/completeness-findings.json", "evaluated_digest": "a87513e0ed417ea678374665baa176e4a2272dcd5aaa730aa658e2e339e2a635"}
...
認証側の駆動要因は、守る対象が「事業と家計を合算した財務データそのもの」であることと、業務データを
有効な利用者アカウント間で共有することである。この条件下で境界を増やすと、得られる利得に対して失う
...
- `status=active` の admin/member は共有業務テナントの本機能を利用できる。admin 権限は利用者管理だけに
  用い、トータル収支には専用のロール差を増やさない。
- `DuplicateVerdict` は共有テナントの現在状態だけを保持し、判断者actorの変更履歴監査は現時点で持たない。
...
認証まわりの構成変更を伴わない。追加 API を既存ゲートの内側へ置くだけで、認証シークレットは
既存の `SESSION_SECRET` から増やさない。
