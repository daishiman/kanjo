...
title: "現金入力 — 追加のみの migration 0050 で owner・transit_purpose・deleted_at を足し、既存行を書き換えずに論理削除と名義を持つ"
project_id: "kanjo"
domain: "database"
status: "active"
priority: null
...
現金入力 — 追加のみの migration `0050` で `cash_entries` に `owner` (名義)・`transit_purpose` (業務の目的)・`deleted_at` (論理削除) を足し、`(user_id, deleted_at)` の索引を張る。既存行は書き換えない。入力経路は列を持たず `transit_from` の有無から導く。`system-spec/database.md` は承認時入力、本書は列・索引・読取条件・保持期間・移行の制約を持つ。
...
- Constraints: D1 (SQLite)。SQLite の `ALTER TABLE ... ADD COLUMN` は列の追加だけを行い既存行を書き換えない。migration は 0050 から採番し、既存行と `cashToDeal` / `cashToTx` の結果を変えない (C2)。
...
|---|---|---|---|---|
| `migrations/0050_*.sql` | 列 3 本と索引 1 本の追加 | SQL | D1 | Migrate ワークフロー |
...
0050 は `ALTER TABLE cash_entries ADD COLUMN` 3 本と `CREATE INDEX` 1 本だけで、UPDATE を含まない。`schema-guard.ts:4` の `EXPECTED_D1_MIGRATION` を 0050 の migration ファイル名へ進める (列の一覧ではなく migration 名で判定する。system-spec と一致)。列の存在は `PRAGMA table_info` で確かめるテスト (前例 `deletion-schema.test.ts:35`) で補う。夜間 cron の完全消去 job は `/api/*` に掛かる `runtimeSchemaGuard` の外で動くため、0050 の適用 (Migrate) を Worker の配備 (Deploy) より先に行う既存の順序で守る。JSON 復元の列リスト (`import-lifecycle.ts:1552` の `restoreCashEntryStatements`) に 3 列を足し、エクスポートと復元で削除中の状態と名義が往復するようにする (qa-cash-database-web-002)。

#### Data verification

migration の適用後に `PRAGMA table_info(cash_entries)` で 3 列、`PRAGMA index_list` で索引を確かめる。既存行の `cashToDeal` / `cashToTx` の結果が 0050 の前後で同じであることをテストで確かめる。
...
- Build/deploy topology: 既存の Migrate ワークフロー (`.github/workflows/migrate.yml`) で 0050 を適用してから Deploy (`deploy.yml`)。
- Migration sequence: 0050 の SQL → `schema.ts` の型 → `EXPECTED_D1_MIGRATION` → 読取条件 → JSON 復元の列 → 完全消去 job。
- Rollback trigger/procedure: Worker を戻しても 0050 の列は落とさない (列の削除は行わない)。旧 Worker は `deleted_at` を読まないため、差し戻し中は削除中の行が再び見える。差し戻しの前に削除中の件数を確かめる。

## Risks and verification

- Risk/assumption: 完全消去 job は `runtimeSchemaGuard` の外で動くので、0050 未適用の D1 に新 Worker の cron が当たらないよう Migrate → Deploy の順を崩さない。
- Risk/assumption: `runtimeSchemaGuard` は列を見ないため、0050 の名前だけ記録されて列が欠ける状態は検出しない。PRAGMA のテストで補う。
- Architecture fitness test: 0050 に UPDATE / DELETE / DROP が無いこと。`cash_entries` の読取に `deleted_at` 条件の無いものが完全消去 job 以外に無いこと。
