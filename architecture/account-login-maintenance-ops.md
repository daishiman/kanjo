...
- リスク: 本番へ資格情報を入れる手作業で、ハッシュ入り SQL が端末出力やディスクに残る → 投入は `docs/runbooks/scripts/admin-credential-remote.sh` の一経路に限る。接続確認を先に行い、stdin だけでパスワードを渡し、出力のハッシュを伏せ字にする (2026-09-15 の break-glass で `pnpm exec` が SQL を表示した事象への対処)。
