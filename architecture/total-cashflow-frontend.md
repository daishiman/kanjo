...
confirmation_evidence: {"evaluator": "system-spec-harness:assign-system-spec-completeness-evaluator", "evidence_ref": "system-spec/archive/2026-09-10-retire-tax-receipt-and-clarify-freee-only/completeness-findings.json", "evaluated_digest": "6d9a3099e81f6ccae5e93105dd5b41d485bcc4a57a9c478239b8f6e694ddacb2"}
...
クエリを無効化して再取得し、一覧表の数字と要確認件数を同時に更新する。

「同じ」判断は付替ではなく消し込みである。要確認に残っている間は freee 側と MF 側が両方数えられて
いるので、判断した瞬間に MF 側が落ち、総額はその二重計上分だけ**減る**。支出なら当該 MF 明細が
家計費から外れて freee 側 1 件だけが事業費に残り、収入なら当該 MF 入金が家計収入から外れて freee
側 1 件だけが事業収入に残る。減った総額は誤りではなく、二重に数えていた分が解けた結果である
(実測: 収入側で 総収入 200,000 → 100,000、事業収入は 100,000 のまま)。恒等式
総支出 == 事業費 + 家計費 / 総収入 == 事業収入 + 家計収入 は判断の前後どちらでも成立する。
恒等式が崩れて見えた場合は消し込み側の欠陥であり、UI 側で辻褄を合わせない。
