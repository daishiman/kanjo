...
検証の入口は既存の CI と `verify:full` だけで、`packages/web/package.json` に `check:cash-screen` (`KANJO_VISUAL_SCOPE=cash node scripts/check-financial-visuals.mjs`) を足し、ルートの `verify:full` の検査列に加える。migration 0052 の反映は既存の Migrate → Deploy の手順に載せる。
...
既存のテスト・`skills:test`・初期 JS 予算・`check:cash-screen` のどれかが赤になったら差し戻す。差し戻しは Worker と web の配信を戻すだけで、0052 の列は落とさない。

#### Infrastructure verification

`verify:full` の緑と、Migrate の手順で 0052 が `d1_migrations` に記録されることを確かめる。夜間ログに `cash_soft_delete_purge` の件数が出ることを本番の初回実行で確かめる。
