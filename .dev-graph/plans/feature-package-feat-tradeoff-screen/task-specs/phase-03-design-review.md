# System task overlay: 試算契約・API 契約・候補キー・データ保存範囲の独立レビュー
...
P02 の決定記録を spec と architecture 8 領域に照らして独立にレビューし、候補キーの区切り文字と上限値など P03 担当の持ち越し事項を確定する。

## 背景

候補キーは科目×取引先から組み立て、PUT の経路パラメータと POST の keys の両方に現れる。区切り文字と長さ上限 (300) が揺れると 422 の判定と保存済みの上書きの照合が壊れる。保存するのは条件と上書きだけで、候補・推定値・差額は保存しない。
...
- Acceptance:
- 候補キーの区切り文字と長さ上限が確定し決定記録にある (OI-02)。
