# System task overlay: 受入条件を検証へ写す試験設計

> **正本ではない。** 本書は system-dev-planner が生成した staging snapshot で、promotion 済みの正本は tasks/feat-improvement-request/SYS-IMPREQ-P04.md である。内容の更新はそちらへ入れる。
...
- Write scope: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-04-test-design.json
...
- Automated commands: 受入条件と検証の対応表を作り、検証が割り当てられていない受入条件が0件であることを確認する / 設計した検証が既存の vitest 構成 (packages/web は jsdom、packages/api は Workers) の上で実行可能な形であることを確認する
