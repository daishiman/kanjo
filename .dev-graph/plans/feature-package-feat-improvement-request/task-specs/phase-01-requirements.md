# System task overlay: 改善要望の実行要件ベースライン確定

> **正本ではない。** 本書は system-dev-planner が生成した staging snapshot で、promotion 済みの正本は tasks/feat-improvement-request/SYS-IMPREQ-P01.md である。内容の更新はそちらへ入れる。
...
- 先行 phase は無い。features/feat-improvement-request.md が confirmed であることが起点となる
...
- Write scope: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-01-requirements-baseline.json
...
- Automated commands: 受入条件15件とベースライン項目の対応表を作り、対応先が空の行が0件であることを確認する / 参照した system-spec と architecture の digest がベースラインに記録され、現行ファイルの digest と一致することを確認する
