# System task overlay: 検証の実行と失敗の解消

> **正本ではない。** 本書は system-dev-planner が生成した staging snapshot で、promotion 済みの正本は tasks/feat-improvement-request/SYS-IMPREQ-P06.md である。内容の更新はそちらへ入れる。
...
- Write scope: packages/web/src/improvement-capture.dom.test.tsx、packages/web/src/diagnostics-buffer.test.ts、packages/api/src/improvement-lifecycle.test.ts、packages/api/src/improvement-redaction.test.ts、packages/api/src/improvement-backup-exclusion.test.ts
...
- Automated commands: pnpm test をリポジトリ全体で実行し、失敗が0件であることを確認する / 実行前後の検証件数を比較し、既存検証の欠落が0件であることを確認する
