# System task overlay: 復旧後の稼働確認と事後観察
...
- phase_ref: P13 (本 package 内で 1 件のみ)
- classification: confidence 1.0。理由は本 task が dev-graph feature の配下で単一 phase の実行単位として生成されたため。候補 path は tasks/feat-prod-d1-schema-recovery/SYS-PDSR-P13.md
...
復旧の効果が一時的なものでないことを、翌日以降の夜間バックアップ成功と主要経路の正常応答で確認し終えている状態。

## 背景

今回の障害では夜間バックアップも同じ原因で毎晩失敗していた。適用直後の正常応答だけを見て完了とすると、周期的に走る処理の回復を見落とす。
...
- Backend: 該当。アプリケーションコードの変更を伴わない。
...
- Produced artifacts: 復旧後の稼働確認結果と夜間バックアップの成功記録を phase 固有 evidence に残す。追加デプロイやrunbook/script変更は行わない。
- Consumed artifacts: arch-d1-schema-lifecycle の設計判断、feature feat-prod-d1-schema-recovery の受入条件
- Write scope/touches: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-13-observation.md
...
- 新機能の投入。追加のデプロイ。監視基盤の新規導入。

## Verification and evidence

- Automated commands: 復旧後の取込と取込履歴の経路が継続して正常応答を返すことを確認する / 夜間バックアップが成功しスナップショットが保存されていることを確認する
- Required evidence: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-13-observation.md

## Rollout and rollback

- Rollout: 追加のデプロイを伴わず、稼働状態の観察のみを行う。
- Rollback trigger and steps: 異常を検知した場合は稼働を止めず、取得済み証跡をもとに原因を切り分けたうえでユーザーの判断で対処する。

## Handoff

- Executor: dev-graph が登録した task ノードを worktree lease のもとで実行する。本番への書込みコマンドはユーザーが実行する
- Ready when: system-spec completeness が PASS で、P12 handoff が完了し、参照元の blocked が解除されていること
...
- Phase doc: 生成側 13 phase 表の P13 (release-deploy)
- Dependencies: task-graph node SYS-PDSR-P12 (直前 phase の完了を前提とする)
- Secondary workstreams: backend
