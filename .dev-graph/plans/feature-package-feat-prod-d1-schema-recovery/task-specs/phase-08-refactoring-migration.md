# System task overlay: 既存データの互換確認と派生テーブルの初期状態
...
- phase_ref: P08 (本 package 内で 1 件のみ)
- classification: confidence 1.0。理由は本 task が dev-graph feature の配下で単一 phase の実行単位として生成されたため。候補 path は tasks/feat-prod-d1-schema-recovery/SYS-PDSR-P08.md
...
新規に作られたテーブルの初期状態が意図どおりであり、既存データが新スキーマのもとで従来どおり読めることが確認されている状態。

## 背景

未適用だったマイグレーションには新規テーブルの追加が含まれる。作成直後は空であるため、集計や履歴の表示が空を異常として扱わないかを確認しておく必要がある。
...
- Produced artifacts: 新規テーブルの初期状態と既存データの読み出し確認結果を phase 固有 evidence に記録する。runbook/script は変更しない。
- Consumed artifacts: arch-d1-schema-lifecycle の設計判断、feature feat-prod-d1-schema-recovery の受入条件
- Write scope/touches: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-08-compatibility.md
...
- データの補正投入。過去分の再集計。スキーマの追加変更。

## Verification and evidence

- Automated commands: 新規作成されたテーブルが空である場合に画面と API が異常終了しないことを確認する / 既存データを持つテーブルの読み出しが従来どおり成功することを確認する
- Required evidence: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-08-compatibility.md

## Rollout and rollback

- Rollout: 確認結果を作業記録へ追記するのみ。
- Rollback trigger and steps: 本 task は読み取り確認のみで状態を変えないため、記録の破棄で戻る。
...
- Phase doc: 生成側 13 phase 表の P08 (refactoring-migration)
- Dependencies: task-graph node SYS-PDSR-P07 (直前 phase の完了を前提とする)
