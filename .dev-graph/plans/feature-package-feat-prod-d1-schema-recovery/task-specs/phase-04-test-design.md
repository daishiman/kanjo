# System task overlay: 行数突合と動作確認の検証設計
...
- phase_ref: P04 (本 package 内で 1 件のみ)
- classification: confidence 1.0。理由は本 task が dev-graph feature の配下で単一 phase の実行単位として生成されたため。候補 path は tasks/feat-prod-d1-schema-recovery/SYS-PDSR-P04.md
...
適用前後で何をどう比較すれば「データが失われていない」と言えるかが、実行可能な検証手順として確定している状態。

## 背景

成功基準にデータ喪失ゼロを掲げても、比較対象と判定式が曖昧なら合否を判定できない。適用前に基準値を取得しておかないと後から比較できない点も設計に織り込む必要がある。

## 前提条件

- Required spec/architecture/phase/task nodes: arch-d1-schema-lifecycle および feature ノード feat-prod-d1-schema-recovery
- Entry gate: P01 の保全対象テーブル一覧が確定していること。
...
- Produced artifacts: 行数突合スクリプトの仕様と、取込および取込履歴の動作確認契約を phase 固有 evidence に記す。runbook/script は変更しない。
- Consumed artifacts: arch-d1-schema-lifecycle の設計判断、feature feat-prod-d1-schema-recovery の受入条件
- Write scope/touches: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-04-verification-design.md
...
- 性能試験。負荷試験。本番データの内容そのものの検査。

## Verification and evidence

- Automated commands: 突合対象テーブルが保全対象一覧と一致していることを確認する / 判定式が行数の非減少であり、増加を異常としないことを確認する / 取込と取込履歴の動作確認が応答コードのみで判定でき、明細内容を参照しないことを確認する
- Required evidence: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-04-verification-design.md

## Rollout and rollback

- Rollout: phase 固有 evidence の追加のみ。
- Rollback trigger and steps: 検証設計節を差し戻す。本番状態には影響しない。
...
- Phase doc: 生成側 13 phase 表の P04 (test-design)
- Dependencies: task-graph node SYS-PDSR-P03 (直前 phase の完了を前提とする)
