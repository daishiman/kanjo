# System task overlay: ローカル環境での適用予行
...
- phase_ref: P06 (本 package 内で 1 件のみ)
- classification: confidence 1.0。理由は本 task が dev-graph feature の配下で単一 phase の実行単位として生成されたため。候補 path は tasks/feat-prod-d1-schema-recovery/SYS-PDSR-P06.md
...
本番と同じ連番のマイグレーションをローカル D1 に対して適用し、手順書と突合スクリプトが想定どおり動くことを確認し終えている状態。

## 背景

本番で初めて手順を試すと、手順の誤りがそのままデータへの不可逆な影響になる。ローカルで一度通しておくことで、手順の欠落とコマンドの誤りを本番前に除ける。

## 前提条件

- Required spec/architecture/phase/task nodes: arch-d1-schema-lifecycle および feature ノード feat-prod-d1-schema-recovery
- Entry gate: P05 の手順書と突合スクリプトが存在すること。
...
- Produced artifacts: 予行の実行ログと検出した不備を phase 固有 evidence に記録する。runbook/script の修正が必要なら唯一writerの P05 へ差し戻す。
- Consumed artifacts: arch-d1-schema-lifecycle の設計判断、feature feat-prod-d1-schema-recovery の受入条件
- Write scope/touches: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-06-local-rehearsal.md
...
- Automated commands: ローカル D1 に対して migrations apply を実行し、全マイグレーションが連番順に適用されることを確認する / 適用後に取込と取込履歴の経路が正常応答を返すことをローカルで確認する / 突合スクリプトを適用前後で実行し、行数の非減少判定が機能することを確認する
- Required evidence: .dev-graph/plans/feature-package-feat-prod-d1-schema-recovery/evidence/phase-06-local-rehearsal.md

## Rollout and rollback

- Rollout: ローカル環境のみで完結する。
- Rollback trigger and steps: ローカル D1 を初期化して再作成する。本番状態には影響しない。

## Handoff

- Executor: dev-graph が登録した task ノードを worktree lease のもとで実行する。本番への書込みコマンドはユーザーが実行する
- Ready when: system-spec completeness が PASS で、P05 の成果物が存在し、参照元の blocked が解除されていること
...
- Phase doc: 生成側 13 phase 表の P06 (test-run)
- Dependencies: task-graph node SYS-PDSR-P05 (直前 phase の完了を前提とする)
