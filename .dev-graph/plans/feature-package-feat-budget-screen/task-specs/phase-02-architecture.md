# System task overlay: budgetScreen・applyBudgetInputs・monthlyBudgetsAt・3 API・migration 0048 のワークストリーム設計決定記録
...
budgetScreen・applyBudgetInputs・monthlyBudgetsAt の入出力契約、3 API (GET /api/budget-screen・GET/PUT /api/budget-plans) の応答形、migration 0048_budget_plans.sql の設計を、実装前のワークストリーム設計決定として docs に固定する。
...
Open risk: migration 0048 の番号は予定であり、SYS-BUDGET-P05 の着手時と SYS-BUDGET-P13 の配信時にそれぞれ origin/main と突き合わせることを設計決定として明記する。
...
- Data: applicable: migration 0048 の追加のみ設計を固定する
...
- migration 0048_budget_plans.sql (budget_plans の新設、主キー (user_id, period_start, account)) が追加のみである方針と、番号が予定番号であり着手時に origin/main と突き合わせる旨が docs に明記されている。
