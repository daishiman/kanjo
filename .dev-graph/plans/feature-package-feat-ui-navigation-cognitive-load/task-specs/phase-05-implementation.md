# System task overlay: 共通サイドバーと低認知負荷UIの実装

> **正本ではない。** 本書は`system-dev-planner`が生成したstaging snapshotで、promotion済みの正本は`tasks/feat-ui-navigation-cognitive-load/SYS-UINAV-P05.md`。内容の更新はそちらへ入れる。本書は生成時点の記録として保持し、正本との差分は意図的なもの(promotion後の追記)を含む。
...
- Produced artifacts: packages/web/src/routeMetadata.ts、packages/web/src/components/Layout.tsx、packages/web/src/components/RouteIcon.tsx、packages/web/src/styles.css、packages/web/src/navigation-ux.dom.test.tsx
...
- Write scope: packages/web/src/routeMetadata.ts、packages/web/src/components/Layout.tsx、packages/web/src/components/RouteIcon.tsx、packages/web/src/styles.css、packages/web/src/navigation-ux.dom.test.tsx
...
- Automated commands: navigation DOM test で /tax と /tax/receipts の active が常に1件であることを確認する / 全 route metadata に意味的 icon key があり、装飾 icon が aria-hidden であることを確認する / 狭幅・キーボード・reduced motion の表示契約を確認する
