/**
 * 『用語と目安』のトピック (spec-guide-screen FR-14)。
 * 旧 指標ガイド画面の内容 (ベンチマーク・略語の読み方・用語の節・データ充足度) をそのまま移し、全用語を 1 度ずつ出す。
 * 現在値は取得できなくても、用語の意味と目安は先に読める。
 */
...
const JUDGE_PILL: Record<string, string> = {
  目安内: 'pill calm',
  目安外: 'pill warn',
  データ不足: 'pill neutral',
};

export function GuideTerms() {
  ...
  return (
    <div className="guide-terms">
      ...
      <section className="scroll-x" aria-labelledby="guide-bench-title">
        <h3 id="guide-bench-title">ベンチマーク(いまの数字と目安)</h3>
        {sq.isLoading ? (
          ...
          >
            {bench.map((b) => (
              <tr key={b.id}>
                ...
                <td>
                  <span className={JUDGE_PILL[b.judge]}>{b.judge}</span>
                </td>
      ...
      */}
      <section className="scroll-x" aria-labelledby="guide-abbr-title">
        <h3 id="guide-abbr-title">略語の読み方</h3>
      ...
      */}
      {sections.map((sec) => (
        <section key={sec.title} className="scroll-x">
          <h3>{sec.title}</h3>
      ...
      <section aria-labelledby="guide-coverage-title">
        <h3 id="guide-coverage-title">データ充足度チェック</h3>
        <table
          className="data"
          style={{ maxWidth: 620 }}
