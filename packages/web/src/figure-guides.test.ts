...
const matrixSource = readFileSync(new URL('./pages/analysis/matrix/MatrixPage.tsx', import.meta.url), 'utf8');
...
describe('図・表の見方', () => {
  ...
  it('Matrixでも説明を図より先に置く', () => {
    expect(matrixSource.indexOf('<HowTo id="matrixSkew" />')).toBeLessThan(
      matrixSource.indexOf('<SkewTop3 data={m} />'),
    );
  });
});
