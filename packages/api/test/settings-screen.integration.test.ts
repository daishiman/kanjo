 ...
 * ## 置換前に落ちる理由
 *
 * 置換前は settings_norm_rules・settings_cash_overrides・settings_change_log (0054〜0056) も
...
describe('AT-17 migration 0054〜0056 は追加だけで、旧表の値を同じ意味で写す', () => {
  const names = migrationFiles();
  const settingsMigrations = names.filter((n) => /^005[4-6]_/.test(n));

  it('0054〜0056 は UPDATE・DELETE・DROP・ALTER を含まない', () => {
