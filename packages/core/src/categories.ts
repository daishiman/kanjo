 ...
 *
 * - 事業(biz): freee の勘定科目(取込済みの freee 取引に実在する科目 + 設定で追加した事業科目)。中項目は持たない
 *   (freee の勘定科目は決算書の科目そのもの。品目列は任意入力のため候補にしない)。
 * - 個人(per): MF の大項目/中項目(取込済みの MF 明細に実在する組み合わせ + 設定で追加した家計科目)。
 *
 * 科目を推測で創作しない。候補に無い科目は利用者が系統を指定して追加する。
 */
...
function buildScope(
  ...
  const add = (big: string, mid: string, source: CandidateSource) => {
    ...
    if (!entry) {
      ...
      majors.set(b, entry);
    } else if (source !== 'custom' && entry.source === 'custom') {
      entry.source = source; // 実データに現れたら「追加」扱いをやめる
    }
    if (m && !entry.mids.has(m)) entry.mids.set(m, source);
  };
...
export function categoryRejectReason(cls: Cls): string {
  return cls === 'biz'
    ? '事業の明細の科目は、freeeの勘定科目(決算書の科目)から選んでください。無い科目は「事業の科目」として追加できます'
    : '個人の明細の科目は、MFの大項目/中項目(家計の内訳)から選んでください。無い科目は「家計の科目」として追加できます';
}
