 ...
 * いまは取り込む先が無いので、画面ではこの一覧を出して「何を取れば作れるか」だけを示す。
 *
 * 並びは「取る順番」であって、重要度順ではない。
 * 銀行口座の残高(step 1)を先に置くのは、MF に全口座が連携されていれば
...
export const BALANCE_SHEET_SOURCES: readonly StatementSource[] = [
  {
    ...
    columns: ['日付', '保有金融機関', '種類', '残高'],
    use: 'MFに連携した全口座の月末残高。BSの現預金と手元資金はここから入ります。',
    note: '無料プランではCSV出力ができないことがあります。その場合は下の口座別で代用します。',
  },
  {
    step: 2,
    name: '口座別の入出金明細',
