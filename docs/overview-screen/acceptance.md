...
## 判定の更新 (main 取り込み後、2026-09-15)

main (PR #50) を取り込んだ後の再実行 (`test-run.md` §5) で、次の 2 つが揃いました。

- AC-005: `check-financial-visuals.mjs` が全幅で合格 (exit 0)。
- AC-007: core 605 / api 553 / web 589 pass、`pnpm typecheck`・`pnpm lint`・`pnpm run test:aux` が exit 0、js 予算 109.29 KiB / 110 KiB。上の既知の 3 件は再現しません。

したがって AC-001..AC-007 はすべて合格で、全体判定は **合格** です。
