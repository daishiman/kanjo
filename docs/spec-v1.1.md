...
- 複数事業・複数ユーザーのマルチテナント
- 確定申告の準備、申告書類の生成・適法性の保証(freee/e-Taxおよび利用者・税理士側の責務とする)
- 領収書・証憑の取込と保管(外部の正本側で管理する)
...
| 用語 | 定義 |
|---|---|
| 明細(Transaction) | 日付を解釈できるMFの1行。全行を保存し、計算対象=1かつ振替=0の行だけを収支集計へ含める |
| 仕訳(Deal) | freeeの1取引行 |
| 公私仕分け(Classification) | 明細を biz(事業)/per(個人) に判定すること。優先順位: 手動 > ルール > 取引先メモリ > **MF中項目の「事業」前方一致** > 既定(per) |
| 中項目による事業判定 | MFの中項目が「事業」で始まる明細を、収入・支出の別なく事業(biz)とみなす規則。前方一致なので「事業経費」「事業・副業」「事業・情報サービス」など利用者が中項目を増やしてもコード変更なしに追従する。**家計側に「事業」で始まる中項目を作らないことが利用条件**(運用約束) |
...
| P2 | 支出分析 — マトリックス | `/analysis/matrix` | 科目×月で「増えた/減った」を特定する(詳細は[`matrix.md`](matrix.md)) |
...
| P11 | トレードオフ | `/tradeoff` | 新しい支出を増やすとき、どの事業経費を見直して捻出するかを試算する |
| P12 | AI分析 | `/ai` | 指示文を作り、AIによる分析結果を読む |
| P13 | 現金の記帳 | `/cash` | 口座・カード明細に出ない現金の受け渡しを記帳する(再取込しても残る) |
| P14 | 支出分析 — 支出トレンド | `/analysis/trends` | 事業・家計の支出を並べ、次に手を打つ科目を決める |
| P15 | 決算書 | `/statements` | 損益計算書・キャッシュフロー・貸借対照表を確認する |
| P18 | 支出分析 — 支出照合 | `/analysis/reconciliation` | freee帳簿確定とMF未記帳を照合し、二重なしの実質支出と次の確認を決める |
| P19 | 支出分析 — トータル収支 | `/analysis/total-cashflow` | 事業と家計を合算し、取込の全分割と月次収支を検算する |

**画面単位とルート数の関係(2026-09-08更新)**: P2・P3・P14・P18・P19は独立したルートではなく、**支出分析(`/analysis/:tab`)の5タブ**である。したがって`APP_ROUTES`は**13本**、画面単位は**13ルート+5タブ=18**となる。5タブはページ内とサイドバーの子行から選べる。P16・P17は廃止済みで、P番号は履歴参照を壊さないため振り直していない。本数の正本は`packages/web/src/routeMetadata.ts`である。

- サブスク分析(P4)は統合していない。集計単位が勘定科目でなく支払先であり、支払先マスタの編集という書き込み操作を持つため。
- 旧URL `/matrix` `/diagnosis` `/trends` は各タブへ`replace`でリダイレクトする(ブックマーク互換)。
- 統合の判断理由は`docs/ui-decisions.md`の「決定の更新(2026-08-30 / ウェーブ4)」に、ナビゲーションの契約は`docs/ui-navigation-guidelines.md`と`specs/ui-navigation-cognitive-load.md`に置く。
...
- 日付を解釈できるMF明細は全行を保存し、`計算対象`・`振替`・`メモ`の原文属性を保持する。収支集計は計算対象=1かつ振替=0だけを含める。金額の正=収入、負=支出
- freee仕訳は発生日から月を導出。科目正規化マップを適用する
- freee複合行の発生日空欄は、直前の有効行と収支区分が一致する場合だけ日付等を継承する。先頭空欄・不正日付後・収支区分逆転では継承せず、決済金額は継承しない
...
- 判定根拠の表示: 各明細に `手動` / `ルール` / `中項目` / `既定` のいずれかを出す。中項目由来は「確認済み」の内訳に数え、未確認件数からは外す(人が見なくても根拠が確定しているため)
...
- 支出照合では、freee事業経費を帳簿確定、MF事業支出のうちfreeeと未照合のものを未記帳、その合計を実質支出として区別する。自動照合は用途・日付・絶対金額・厳格正規化した支払先が1対1で一致し、MF安定IDがあり非分割の場合に限る。曖昧候補は統合せず要確認へ残す
- マトリックス: 金額 / 前月比 / 前年同月比 の3モード、合計・平均の行列、金額モードの7階級ヒートマップ(分母は表全体で共通・本体セルのみ)、偏りが大きい3点、CSVダウンロード。未記帳月は合計・平均・比率・濃淡のすべてから除外する。集計規則の正本は[`matrix.md`](matrix.md)
- 診断: 検知器レジストリ(core の1か所)が改善余地を検知し、年間影響額の降順で改善アクションを並べる。規則ベースの健全性スコア(固定費比率・貯蓄率・安定性・予算カバー率の4要素、重み 30/30/25/15、寄与点の合計が総合点)、改善インパクトのウォーターフォール、主なシグナル(最大3件)、検知根拠の明細を同じ1画面に置く。従来の科目別統計プロファイル(平均・中央値・σ・CV・基準レンジ・傾き・zスコア・費目分類)は根拠タブへ残す。判定規則と検知器の追加手順の正本は[`diagnosis-screen.md`](diagnosis-screen.md)
- サブスク: 入力元を固定せず照合後の実質支出から、ベンダー別月次積み上げ、重複検知(月額が中央値の1.8倍超かつ2万円超)、急増検知(3倍超かつ1.5万円超)を導出する。画面の KPI・一覧・詳細・見直し候補 (理由文つき、確認済み/対象外の判断と取消)・別名の統合は core の `subscriptionsScreen` 1 か所から導き、一覧の合計行・比較の合計・KPI の月額合計を一致させる。集計規則と見直し候補の規則の正本は[`subscriptions-screen.md`](subscriptions-screen.md)
...
**FR-05 エクスポート(P9)**
- 統合データJSONのダウンロードと、HTML版互換JSONから集計・設定を初期移行する。現金明細は移行先が空かつ49 query予算内の場合だけidを保持して復元する
...
- 入力: 新しい支出の支出名・金額(1〜1億円の整数)・頻度(単発 / 毎月)・開始月・メモ。開始月は記録用で試算に効かない
- 出力: 見直し候補(事業経費の科目×取引先)、推奨の組み合わせ(上位4件)、試算結果(年額・削減額・差額・防衛ラインへの影響)
- 規則の正本は `packages/core/src/tradeoff-screen.ts`。api と web は年額・差額の式を持たず、core を呼ぶだけにする。画面仕様は `specs/spec-tradeoff-screen.md`

| 規則 | 内容 |
|---|---|
| 候補 | 期間の終了月から遡る暦の3か月の freee 経費を科目×取引先で集計し、3で割った月額(四捨五入)が1,000円以上のもの。月額の降順、同額は候補キー(`v1:` + JSON 配列 `[科目, 取引先]`、科目は正規化後)のコード単位順で最大50件。キーが300字を超えるものは候補にしない |
| 推移 | 3か月の最初の月と最後の月の額で決める。最後 > 最初×1.1 は増加、最後 < 最初×0.9 は減少、±10%ちょうどは横ばい。最初の月が0なら最後が正で増加、0で横ばい |
| 必要度(推定) | 上から順に最初に当たった行を採る。固定費で推移が減少以外 → 高 / スポット → 低 / 推移が減少 → 低 / それ以外 → 中。利用者が上書きした値があればそれを採り『手動』と示す(自動に戻せる) |
| 理由 | 利用者のメモがあればメモ。無ければ「{固定費/準変動/スポット}・直近 3 か月は{横ばい/増加/減少}」に、診断の改善案に当たった科目はその見出しを「・」で足す。当たった科目は改善案の関連ページへリンクする |
| 試算 | 新しい支出の年額 = 毎月なら金額×12、単発なら金額そのもの。削減額(年間) = 選んだ候補の月額合計×12。差額 = 年額 − 削減額。差額 ≤ 0 は「捻出できる」、正は「支出増」。税・手数料は考慮しない |
| 推奨の順位 | 月額上位12件から2〜4件を選び、削減額(年間)が新しい支出の年額以上になる組み合わせを、必要度 高 の件数の少ない順 → 低 の件数の多い順 → 超過額の小さい順 → 件数の少ない順 → キー順で並べ上位4件。充足度 = 削減÷年額の%(切り捨て)。しやすさは 低 が半数以上で易しい・1件以上で普通・0件で難しい。リスクは 高 が0件で低・1件で中・2件以上で高 |
| 防衛ラインへの影響 | FR-08 の防衛ラインとの月の差(余裕)×12 を年間の余裕とし、試算後 = 年間の余裕 − 差額。0以上なら維持、負なら割れる。記帳済みの月が無い(nodata)ときは判定しない |

- 「この条件で試算」は入力と選んだ候補キーだけを送り、選んだ候補の月額合計(covered)と判定はサーバーが現在の候補から再計算して `tradeoff_plans` に1行追加する。画面は最新の1件を入力と選択へ復元し、今の期間に無い候補は外して件数を知らせる
- 保存済みの試算の一覧と翌月の実績との突合は画面に出さない(突合の数字は core の `tradeoffReview` が持ち続ける)

**FR-10 現金・交通費(P13)**
- 口座・カード明細に出ない現金の受け渡しを、事業/個人・収入/支出・科目とともに記帳する
- 交通費は出発・到着を対で持ち、往復なら片道額の2倍を記帳する。`receipt_waived`はこの交通費入力の互換フィールドであり、領収書・証憑の保管機能を表さない
- 再取込で現金明細を消さず、事業分はfreee形状、個人分はMF形状へ投影して既存集計へ合流させる
...
- MF CSVのID列が空の明細(`identity_stable=0`)は分割不可。再取込を案内する
...
| 可用性 | Cloudflareマネージド範囲に準拠。個人利用のためSLA目標は設けないが、D1の自動バックアップ+日次エクスポート(R2)を行う。R2 cleanupは既存nightly scheduledで全bucket scanなしの有界batch・backoff・dead-letterへ接続する |
| セキュリティ | 全経路HTTPS。Cloudflare Accessまたは署名付きHttpOnlyセッションのどちらかで認証必須。password loginはraw IP/passwordを残さないD1 atomic rate limitで保護する。R2/D1へは Worker 経由のみアクセス(公開バケット禁止)。金融明細のためログに明細内容・金額・R2オブジェクトkeyを出力しない |
...
│  │   ├─ classify.ts      # resolveIncomingTx / resolveTx / applyClassification
...
CREATE TABLE mf_transactions (
  ...
  memo TEXT,                      -- メモ原文。列欠落はNULL、空セルは空文字
...
CREATE INDEX idx_mftx_active_month ON mf_transactions(user_id, month)
...
**比率の共通契約**: 分析・構成比はcore・API・UI間で `1 = 100%` の小数として受け渡す。構成比は `0..1`、増減率は減少時の負値と100%超の増加を許容し、百分率への変換はUI/CSVの表示境界でのみ行う。例外として、利用者が入力・確認する家事按分の `businessPercent` はDB/API/core/UIを通して `0..100` の整数を正本とする。

1. **保存明細と集計対象**: 日付を解釈できるMF行は全て保存し、計算対象=1かつ振替=0だけを収支集計へ含める
2. **公私判定**: `overrides[tx_id]` → ルール先勝ち(内容|大項目|中項目 を大文字化して部分一致) → 取引先メモリ → **中項目が「事業」で始まれば biz** → 既定 per
   - 中項目の前方一致は収入・支出の両方に同じ式を当てる(金額の符号を見ない)。正本は `packages/core/src/types.ts` の `isMfBizByMid` 1 箇所だけで、集計側(`total-cashflow.ts`)は判定をやり直さず `resolveTx` の結果に従う
   - 判定の出どころは `clsSrc` として画面・API へ返す(`手動` / `ルール` / `中項目` / `既定`)。`中項目` は確認済みとして数え、未確認(`reviewPending`)には含めない
...
16. **予算推奨値**: 固定費=直近3ヶ月平均×0.95 / その他=全期間平均(千円丸め)。判定: 差異が予算の±10%超で超過/余裕。ちょうど±10%は範囲内
...
| GET | /business-spend | freee帳簿確定・MF未記帳・重複排除後の実質支出、月別比較、曖昧な照合候補を返す。canonicalは変更しない |
...
| GET | /diagnosis | P3用: 診断画面1枚分(健全性スコアと寄与点、改善アクション、ウォーターフォール、主なシグナル、範囲別合計、科目別統計プロファイル+BEP)。`scope`/`metric`/`range`/`compare` で絞り込む |
| PATCH | /diagnosis/actions/:action_key | 改善アクションへの判断を保存。`status` は `未着手`/`対応中`/`対応済み`/`見送り` の4語完全一致(不一致は400)、`note` は500字まで(超過は400)。判断だけを永続化し、改善余地そのものは毎回計算し直す |
...
| GET/POST | /tradeoff | FR-09用: 見直し候補・防衛ラインの余裕・最新の記録の取得 / 試算条件の記録(covered と判定はサーバーが再計算) |
| PUT | /tradeoff/candidates/:key | FR-09用: 候補ごとの必要度とメモの上書き(両方 null で自動へ戻す) |
| GET | /export/json, /export/matrix.csv | エクスポート。JSONは監査用`cashEntries`とversion付き確定delta`cashProjection`を同梱 |
| POST | /restore | HTML版互換JSONから集計・設定を初期移行。multipart JSONと同じfingerprint/claim/状態/atomic handlerを使うが、直接bodyなのでR2は作らない。validな`cashProjection`だけをbaselineから除外し、現金明細は移行先が空かつ49 query予算内のときだけidごと復元する(`cashEntries`件数)。既存があれば`cashKept`、予算超過なら`cashSkipped`で返す。imported `cash:*` editは破棄し、現存cash用のDB editは保持 |
...
- **ナビゲーション(全ページ共通)**: `APP_ROUTES`13本+`ANALYSIS_TABS`5本(計18単位)の表示名・1タスク文・段階表示の詳細文・アイコンは`routeMetadata.ts`を正本とし、ルート定義・サイドバー・モバイルナビ・画面検索で共用する。現在ページは`aria-current="page"`と、色以外の手掛かり(文字色+左帯)で示し、同時にcurrentとなるリンクを1件以下に保つ
- **モバイルナビ(640px以下)**: `mobileLabel`を持つ最頻4タスク+メニューの下部タブへ切り替え、残りはドロワーに含める
- **画面検索(全ページ共通)**: `Cmd+K` / `Ctrl+K`で画面検索を開き、名前・群名で全画面単位へ直接移動できる。支出分析の5タブも同じmetadataから引く
- **サイドバー行高**: タップ環境(`pointer: coarse`)と640px以下では44px(WCAG 2.5.5 相当)、それ以外の常設幅では36px(SC 2.5.8 の24pxを上回る)
- **共通表示**: ページ見出し・読込/失敗/空状態・KPI・年間比較は安定した共通部品を使い、空と失敗には次の行動を表示する
...
- マトリックス: 先頭列固定・横スクロール。モード切替はセグメントコントロール。濃淡は色だけで判断させず、セルに金額を常に表示し凡例に階級の下限を出す
- サブスク: 一覧の行を選ぶと右に詳細 (概要・支払い履歴・関連データ) を開き、選んだ行を URL の `vendor` に残す。サービスのロゴは取得・表示しない
- 取込: ドラッグ&ドロップ+進捗表示+結果ログ(件数/対象月/スキップ理由)。同月洗い替えの旨を確認ダイアログで明示
- 設定: HTML版JSONは集計・設定の初期移行と明記する
...
main:  上記CIの成功 → D1 pending判定 → Time Travelの復元地点記録
       → 非破壊migration自動適用 → wrangler deploy(production)
       → 30秒後とさらに90秒後の2回スモークテスト
手動:  破壊的migrationだけ承認済みmanifest + APPLYでMigrate
...
- 非破壊のexpand migrationは`Deploy`がTime Travelの復元地点を記録してから自動適用する。破壊的変更は承認済みmanifestと`APPLY`確認付きの`Migrate`だけで適用する
...
2. 本システムの `POST /restore` に投入する。これは集計・設定の初期移行であり、受信fieldは現在値へmergeしてから、MF原本は含有月を洗い替え、rules/edits/institution owners/budgets/cash override/復元baseline/未記帳月は全置換する。sub vendorは移行先固有行を保持しつつ同名のaliases/accountsをsourceで更新し、分析設定と候補除外もdurable intentとして移行する。freee原本と現存現金用editは保持する。現金明細は移行先が空かつ予算内のときだけidごと復元する。API export/backupの `cashProjection` v1をbaselineから差し引き、監査用`cashEntries`から再演算せずsource確定scopeへ戻す。`POST /restore`はR2原本を作らない
...
| D1障害・誤操作 | 夜間R2エクスポートから集計・設定を初期移行し、freee/MF原本を再取込する。現金明細は移行先が空かつ予算内のときだけ復元する |
...
## 15. 参考実装(収支管理ダッシュボード.html)・要件定義書との差分(2026-08-25 時点)

参考資料はローカル専用(`docs/reference/`、実明細を含むため git 管理外)。ここでは「参考実装にあって本アプリでどう扱ったか」を記録する。

| 参考実装の要素 | 本アプリ | 状態 |
|---|---|---|
| 概況: 経費計・集中度(上位2科目シェア)・年間比較 | 概況ページ | 反映済み |
| 増減マトリクス: 科目×月、年合計 | 増減マトリクスページ | 反映済み |
| 診断: CV/z による固定費・準変動・スポット判定、自動診断文、損益分岐点 | 診断ページ | 反映済み |
| サブスク: ベンダー別積み上げ・年間比較・重複/急増検知 | サブスク分析ページ | 反映済み |
| サブスク: 「いま何にいくら」(直近月額・年換算・直近12ヶ月)と運用ルール(月額×12、月3,000円=年3.6万円、契約は月次、解約判断は四半期) | サブスク分析ページ上部の KPI と一覧、補足文 | 反映済み(2026-08-25) |
| 家計: 最新月の個人収入/支出/収支 KPI と注記、月別の収入・支出棒グラフ、1ヶ月だけのときの案内文 | 家計ページ「収支バランス(月別)」 | 反映済み(2026-08-25) |
| 家計: 世帯としての収支(個人+事業入金 − 生活費 − 事業立替)、貯蓄率、生活費の大項目別 合計/月平均/年換算/構成比 | 家計ページ(参考実装より拡張) | 反映済み(2026-08-25) |
| 予算: 予算サマリー(設定済み科目数・予算合計 月/年・実績との差異) | 予算管理ページ上部 KPI | 反映済み(2026-08-25) |
| 指標ガイド: ベンチマーク表(経費率 20〜40% / サブスク対売上比 10〜15%以内 / 安全余裕率 30%以上 / 貯蓄率 20〜30% / 食費比率 15〜20% / 通信費比率 5%以内) | 指標ガイドページ上部(同じ式で算出、判定付き) | 反映済み(2026-08-25) |
| 仕分け: 月別仕分けサマリー、仕分けの考え方の説明枠 | 仕分けページ上部の進捗パネル(`classificationProgress`)と折りたたみ説明枠 | 反映済み(2026-08-27, PR #13) |
...
集計の出どころ(家計ページに1行で表示している内容): 個人収入・生活費・事業入金・事業立替は MF 明細(仕分け画面の結果)、売上・事業経費は freee。口座間の振替は含めない。
