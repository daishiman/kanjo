...
**画面単位とルート数の関係(2026-08-30更新)**: P2・P3・P14は独立したルートではなく、**支出分析(`/analysis/:tab`)の3タブ**である。出口が同じ(見直す勘定科目を1つ選ぶ)ため1画面へ束ねた。したがってルートは**15本**、上表の画面単位は**15ルート+3タブ=18**となる。サイドバーに行を持つのは15本で、タブはページ内のタブ帯から選ぶ。P番号は既存文書からの参照を壊さないため振り直していない。
...
- **ナビゲーション(全ページ共通)**: 15ルート+支出分析3タブ(計18単位)のルート・表示名・1タスク文・段階表示の詳細文・アイコンは1つのmetadataを正本とし、ルート定義・サイドバー・モバイルナビ・画面検索で共用する。現在ページは`aria-current="page"`と、色以外の手掛かり(文字色+左帯)で示し、同時にcurrentとなるリンクを1件以下に保つ
- **モバイルナビ(640px以下)**: 最頻5タスク+メニューの下部タブへ切り替え、残りはドロワーに含める
- **画面検索(全ページ共通)**: `Cmd+K` / `Ctrl+K`で画面検索を開き、名前・群名で画面へ直接移動できる。サイドバーに行を持たない支出分析の3タブもここから引ける
