...
**比率の共通契約**: core・API・UI間の比率はすべて `1 = 100%` の小数で受け渡す。構成比は `0..1`、増減率は減少時の負値と100%超の増加を許容する。百分率への変換はUI/CSVの表示境界でのみ行う。
...
- **ナビゲーション(全ページ共通)**: 15画面のルート・表示名・1タスク文は1つのmetadataを正本とし、ルート定義とナビで共用する。現在ページを強調する
- **モバイルナビ(640px以下)**: 最頻4タスク+メニューの下部タブへ切り替え、残りはドロワーに含める
