...
| `日付` | 月キーへ正規化 |
| `金額` | 正＝収入、負＝支出 |
| `大項目` / `中項目` | カテゴリ。仕分けルールのマッチ対象にもなる。**`中項目` は加えて公私仕分けの判定入力でもある**(「事業」で始まれば事業。後述) |
| `内容` | 明細名。仕分けルールのマッチ対象 |
...
- 事業/個人の区分の根拠: 事業側は freee の取引(売上・経費)、個人側は MF 明細のうち仕分け(手動 > ルール > 取引先メモリ > 中項目の「事業」前方一致 > 既定=個人)で `個人` になったもの。

### 判定根拠 `clsSrc` の値一覧

公私仕分けの結果には、どの根拠で決まったかを表す `clsSrc` が必ず 1 つ付く
(`ResolvedTx.clsSrc` / API 応答の `src` / `progress.bySource` のキー)。
優先順位の高い順に評価し、最初に当たったものが `clsSrc` になる。

| 値 | 意味 | 確認済みか |
|---|---|---|
| `手動` | 利用者が明細ごとに指定した (`tx_edits.cls`)。何よりも優先する | 確認済み |
| `ルール` | 仕分けルールのキーワードが `内容 \| 大項目 \| 中項目` に部分一致した | 確認済み |
| `中項目` | MF の**中項目が「事業」で始まる**。収入・支出の別なく事業とみなす | 確認済み |
| `既定` | どれにも当たらなかった。`cls` は `per` になるが、これは「個人と判断した」ではなく「まだ判断していない」 | **未確認** (`reviewPending` に数える) |

- 判定式の正本は `packages/core/src/types.ts` の `isMfBizByMid` の 1 箇所だけ。
  前方一致なので中項目名の一覧をコードが持たず、利用者が中項目を増やしても実装変更なしに追従する。
- `取引先メモリ` が当たった場合の `clsSrc` は `手動` 系ではなく、既存の `origin: 'vendor_memory'` で表す
  (この feature で列挙値を増やしていない)。
- **運用条件**: 家計側に「事業」で始まる中項目を作らないこと。
  破れると家計の支出が事業へ混ざる。対処と監視は `docs/mf-business-classification/runbook.md`。
...
## 設定画面の 3 表(0051〜0053 / feat-settings-screen)

`migrations/0051_settings_norm_rules.sql`・`0052_settings_cash_overrides.sql`・`0053_settings_change_log.sql` が 3 表を足す。Drizzle の定義は `packages/api/src/db/schema.ts` の `settingsNormRules`・`settingsCashOverrides`・`settingsChangeLog` にある。画面仕様の正本は `specs/spec-settings-screen.md`、判断の経緯は [`settings-screen/design-decisions.md`](settings-screen/design-decisions.md) にまとめた。`schema-guard.ts` の `EXPECTED_D1_MIGRATION` は `0053_settings_change_log.sql` を指す。

### `settings_norm_rules`(集計ルール。0051)
...
### `settings_cash_overrides`(現金上書き。0052)
...
- 0053 は、0051 で移した集計ルールの行を origin `migration`・更新者 `system` として記録する。現金・名義・統計の既存の値の履歴は作らない。

### 旧表からの写し規則(BR-22 系)

0051・0052 の初回の写し、旧 API の書込み(`legacySettingsTarget`)、古いバックアップ本文の読み(`settingsJsonFromBackup`)は、どれも同じ意味で写す。
...
- **旧表は書き換えない**: 0051〜0053 には、`account_norm_map`・`cash_overrides` への DROP・ALTER・UPDATE・DELETE が無い。
