...
画面の数字と規則が食い違う。毎回 `duplicateMatchScore` で計算する。

## 2. 既存の除外理由が読めること (FR-004)

移行 `0042` より前の `freee_deal_exclusions` は自由文の `reason` しか持たない。
...
`移行 0042 より前に保存された除外 (FR-004)`。

このテストは **0041 までを適用した DB に旧形式の行を入れてから 0042 を当てる**形で書いた。
...
| バックアップ | 復元後 |
|---|---|
| 3 表の key を持たない (0042 以前に取ったもの) | 既存の判断がそのまま残る |
...
先行する migration が既に使っていた「復元先が空なら `DELETE` を撃たない」やり方を、
