...
| # | 規則 | 根拠 | 実装 | テスト |
|---|---|---|---|---|
| R28 | migration 0050 は追加だけ。既存の表の UPDATE・DELETE・DROP を含まず、`import_runs` の新しい列は NULL を許す | 利用者 | `migrations/0050_import_inspections.sql` | `packages/api/src/import-migration-0050.test.ts` |
...
| R30 | 配信は Migrate → Deploy の順。0050 を適用する前の Worker は新しい表を読めない | 利用者(運用) | `.github/workflows` の手順 | — (運用の注意。[`design-decisions.md`](design-decisions.md) §9) |
