...
| OI-04 | migration 番号 0050 | P05 | 2026-09-22 に `origin/main`(f0e5a3b)を fetch した時点で最新は `0049_ai_report_invariants.sql`。0050 のまま使う。並行ブランチが先に 0050 を使うと git の衝突には出ないので、merge の前(P13)にもう一度 fetch して番号を確かめる |
...
| 新表の形 | spec のデータモデルどおり | migration 0050 |
...
- 互換性: migration 0050 は未公開・未適用なので、旧 `account|partner` 形式との読み書き互換や移行分岐は持たない。
...
候補キーの衝突指摘は、§4 の versioned JSON tuple と可逆 parser へ変更して解消した。0050 は未公開・未適用のため、旧形式の互換分岐は追加していない。
...
準備までで止めた。commit・push・PR の作成・merge と、本番の Migrate / Deploy は行っていない。merge の前に `origin/main` をもう一度 fetch し、0050 がまだ空いていることを確かめる(OI-04)。
