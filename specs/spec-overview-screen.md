...
- `FR-004` (O3): 月次クローズ 4 ステップを対象月のデータから判定する。データ取込 (対象月を含む committed 取込があり、未記録月でない)・仕分け 0・照合 0・月次レビュー行あり。内容指紋が一致する有効な保留は、未処理キューと月次クローズの両方から除く。指紋が古い保留は除かず再び未処理とする。月次レビューは D1 に保存する。 判定: core 単体テストで 4 ステップの判定表が通り、API テストでバックアップ→全消去→復元の往復後も保留と月次レビューが残る。
...
- `BR-005`: 内容指紋が一致する有効な保留は未処理件数と対象月のクローズ判定の両方から除く。指紋不一致の保留は無効とする (G3)。
