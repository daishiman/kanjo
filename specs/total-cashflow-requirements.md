...
O1-O6 の測り方がそのまま受入条件である。

- 恒等式 `総支出 = 事業費 + 家計費` / `総収入 = 事業収入 + 家計収入` が全月で成立 (契約テスト)。
- (金額, 発生日) ごとに MF から `min(n, m)` 件が事業費側へ寄る。freee 件数を超える付替が 0 件、
  支払先を用いた判定が 0 件、±3 日照合による自動付替が 0 件 (契約テスト)。
- 利用者の「同じ」判断による付替が freee 側 1 取引につき MF 1 件まで。
- 判断が保存され再取込後も同じ明細へ再適用される (結合テスト)。
- `TREND_MIN_MONTHS` 未満は「判定不可」、有意判定は `TREND_ALPHA` を既存と共有する。
- 9 列すべてが常時表示され、取込完了時に要確認が残っていれば画面へ警告が出る。
