...
真の全件初期化は canonical 行だけでなく `restored_monthly_agg` も消すため、
preflight の対象月と指紋に復元baseline行を含める。実行時の `confirmedPeriod` は
サーバが返した `fullRange` の min/max と完全一致する場合だけ受け付ける。
...
単一のクリックで到達させない。期間は画面へ転記させず、preflight が返した `fullRange` を
確認指紋とともに実行APIへ返す。落とすのは内部のテーブル名・SQL・明細の生データで、
これらは画面へ出さない。
...
- **入れ替えの入口**: 取込枠の上部に「データを入れ替える」を1か所だけ置く。独立dialogで all preflight の件数・期間・種別と、freee/MF側の元データ・手入力現金/負債・設定が残ることを示す。成功直後は30日undoと「新しいファイルを選ぶ」を同じ場所に出す。
- **期間削除の入口**: 履歴の後ろの開示領域は期間・種別を絞るメンテナンスに限定する。全件の二重入口は作らない。
...
| POST | `/api/data/deletions/preflight` | 明細・期間・全件の事前確認。種別は期間・全件の任意フィルタ。全件は復元baseline月を含むサーバ導出`fullRange`を返す。範囲はサーバ側で再解釈する。書き込みなし |
| POST | `/api/data/deletions` | 明細・期間・全件の削除。確認指紋必須。全件はpreflightが返した`fullRange`と完全一致する`confirmedPeriod`を必須とする |
| POST | `/api/imports/diff` | 読み取り専用のMF明細差分プレビュー。追加/変更/削除/不変の件数、衝突行の base/current/incoming、自動適用/候補の件数を返す。freee・資産推移・JSONは正常応答 `supported:false` で通常取込へ案内する。`apply=1` は400 `diff_read_only` |
...
- [ ] 「データを入れ替える」はサーバ導出の全期間と指紋を使い、`restored_monthly_agg` だけが残る月も要約とundoの対象に含む
