...
- 追加のみの migration（0054〜）と、`schema-guard.ts` の `EXPECTED_D1_MIGRATION` の更新。
...
- 追加のみの 3 本とする: `0054_settings_norm_rules.sql`（集計ルール）・`0055_settings_cash_overrides.sql`（現金上書き）・`0056_settings_change_log.sql`（変更履歴）（番号の割り方は agent 推定・利用者未確認。根拠 qa-settings-database-web-002。1 本にまとめる案も同じ根拠にある）。
...
- 起案時の予定番号は 0051〜0053 だった。実装の最終段で `origin/main` を fetch し直したところ、トレードオフ画面（#68）が `0051_tradeoff_notes.sql` を使っていたため 0052〜0054 へ繰り上げた。さらに PR を出したあとに現金入力画面（#70）が `0052_cash_entry_owner_soft_delete.sql` を、データ取込画面（#69）が `0053_import_inspections.sql` を使ったので、main を取り込んだ merge で 0054〜0056 へもう一度繰り上げ、`packages/api/src/schema-guard.ts` の `EXPECTED_D1_MIGRATION = '0056_settings_change_log.sql'` と本書の参照を揃えた（C3）。番号の衝突は git の衝突には出ないので、merge のたびに fetch し直して空き番号を確かめる。
- 既存の行を書き換える文（UPDATE・DELETE・表の作り直し）は入れない。既存 `account_norm_map`・`cash_overrides` は残す（C3）。

```sql
-- 0054_settings_norm_rules.sql
...
```sql
-- 0055_settings_cash_overrides.sql
...
- リリース順: Migrate（0054〜0056）→ Deploy（cron の変更を含む）。
...
| ID | 食い違い | 影響 | 本書で関わる箇所 |
|---|---|---|---|
| R-1 | 既存 `cash_overrides` は、画面の空欄が 0 に潰れて保存されている（`PUT /api/settings` の `cashOverrides` は非負整数か null で、空欄は 0 として送られてきた）。 | そのまま『月指定の 0 円』へ写すと、利用者が空欄のつもりだった月の現金集計を 0 円で上書きする。 | §データモデル の 0055 の写し、BR-12・BR-14、Q-4 |
