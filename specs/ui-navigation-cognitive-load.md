...
**画面単位の定義(2026-09-03更新)**: 本仕様の「画面」は**15ルート+支出分析(`/analysis/:tab`)の4タブ=19単位**を指す。増減マトリクス・支出トレンド・統計診断に、入力元を跨ぐ「支出照合」を加えた。以下、本文で「全route」「全単位」と書くときはこの19単位を指し、「15 route」と書くときはサイドバーに行を持つルートだけを指す。判断の理由は`docs/ui-decisions.md`、運用契約は`docs/ui-navigation-guidelines.md`が持つ。
...
| 単位 | サイドバーに行を持つ15 routeと、支出分析の4タブを合わせた19個の画面単位 |
...
- `FR-002`: `APP_ROUTES`(15)と`ANALYSIS_TABS`(4)の全19単位に、意味の異なるアイコン識別子と可視ラベルを持たせる。一意性はアイコンキーではなく**図形の署名**で判定する(キーが違っても見た目が同じなら区別にならないため)。
...
- `FR-008`: 全ページから`Cmd+K` / `Ctrl+K`で画面検索を開き、ラベル・目的文・群名から19単位すべて(サイドバーに行を持たない4タブを含む)へ移動できる。検索対象は`routeMetadata.ts`の正本をそのまま使い、検索側に画面一覧を二重に持たない。
