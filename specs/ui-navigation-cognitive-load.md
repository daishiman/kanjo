...
**画面単位の定義(2026-09-08更新)**: 本仕様の「画面」は`APP_ROUTES`と`ANALYSIS_TABS`に登録された単位を指す。現在のsnapshotは**13ルート+支出分析(`/analysis/:tab`)の5タブ=18単位**である。本数の正本は`routeMetadata.ts`であり、文書の数値を受入判定には使わない。

以下、本文で「全route」「全単位」と書くときはこのmetadata集合を指す。5タブもサイドバーに行を持つ(親「支出分析」の子として展開する)。判断の理由は`docs/ui-decisions.md`、運用契約は`docs/ui-navigation-guidelines.md`が持つ。
...
- `FR-001`: 任意のURLでcurrent itemは最大1件とし、`/analysis/:tab`では子タブだけをcurrentにする。
- `FR-002`: `APP_ROUTES`と`ANALYSIS_TABS`の全単位に、意味の異なるアイコン識別子と可視ラベルを持たせる。一意性はアイコンキーではなく**図形の署名**で判定する(キーが違っても見た目が同じなら区別にならないため)。
...
- `FR-008`: 全ページから`Cmd+K` / `Ctrl+K`で画面検索を開き、ラベル・目的文・群名から全単位へ移動できる。検索対象は`routeMetadata.ts`の正本をそのまま使い、検索側に画面一覧を二重に持たない。
...
- `FR-012`: `/analysis/:tab`にいるとき`aria-current=page`はその子行1件だけに立てる。親「支出分析」には立てない。
...
- `BR-002`: 会計上の警告、保存結果、現在の編集対象を隠さない。
...
- [x] `AC-001`: 任意の登録routeでcurrent navが最大1件で、`/analysis/:tab`では子タブだけがcurrentになる。
- [x] `AC-002`: metadataの全単位にiconと可視labelがあり、図形の署名が一意である。
- [x] `AC-003`: currentが`aria-current=page`を持ち色なしでも識別できる。左帯のコントラストは3:1以上(実測6.46:1)。
- [x] `AC-004`: metadataの全単位で目的・主操作・重要警告が初期表示から失われず、`taskDetail`が段階表示で開く。
- [x] `AC-005`: 編集surfaceで対象、保存、取消、危険性、保存結果を識別できる。
- [x] `AC-006`: unit/DOM/build/UI contract/主要viewport visual確認がPASSする。
- [x] `AC-007`: `Cmd+K` / `Ctrl+K`で画面検索が開き、metadataの全単位を名前と群名で引ける。
...
- [x] `AC-012`: 破壊的操作の確認で`window.confirm`が呼ばれず、画面内に`role="dialog"`が現れ、その本文から失われる対象を読める。やめれば何も送らない。これらの検査は確認を外した実装で落ちることを実測済み。
...
- サイドバー総高はmetadataのroute行と群見出しに、支出分析の子行5本が加わる。ノートPCではスクロールが必要になり得るため、行数を固定値で複製せず実描画検査で監視する。
