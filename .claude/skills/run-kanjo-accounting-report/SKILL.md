...
- **ネットワークが使えない実行環境がある**。Codex のサンドボックスは既定で外向き通信を止めていることがあり、その場合 `curl` は接続エラーで失敗する(トークン切れではないので再発行を頼んでも直らない)。取得の1回目で接続に失敗したら、リトライやプロキシ設定を探らず **手貼り経路**へ切り替える:
  1. 利用者に「AI分析」画面の **「データを表示」** を押して出た JSON を貼ってもらい、それを `${TMPDIR:-/tmp}/kanjo-ai-data.json` に保存して入力にする。
  2. 送信も同じ理由で失敗するので、検査済みの送信JSONをそのまま提示し、画面の **「結果を貼り付ける」** へ案内する。
  この経路でも完了条件は変わらない(検査は必ず `validate-report.py` を通す)。ネットワークの可否は環境の性質であって、利用者の操作ミスではないと伝える。
...
- 接続エラー(名前解決失敗・タイムアウト・`Could not resolve host`) → サンドボックスが通信を止めている。Runtime root contract の**手貼り経路**へ切り替える(「データを表示」の JSON を貼ってもらう)。トークンの再発行は不要。
...
| `split` | 事業/個人・事業/妻/家族の別 | 事業と個人の収支、名義別の収入・支出(`items` **2行以上**)。名義未設定の金融機関があれば明記。個人データが無ければ `gap` |
...
- `401` → 再発行を依頼して止める。
- 接続エラー(送信不能環境) → 検査済みJSONをそのまま提示し、「AI分析」画面の**「結果を貼り付ける」**へ案内する。取得を手貼りで行った場合は送信もここになる。
