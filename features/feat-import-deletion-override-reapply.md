...
implementation_readiness: {"status": "blocked", "missing_sections": ["vendor_memory の confidence 算出式と初期閾値", "balance_entries / cash_entries の由来 import_id の持ち方", "既存明細への base 埋め移行の経路と検証", "undo 退避行の保持期間の具体値"], "checked_at": "2026-08-31T00:00:00Z"}
purpose: "取り込んだあとに取込元が動く以上、帳簿を最新へ収束させる手段が要る。いま削除APIは1本も無く、上書きの判断は実装に固定され、利用者が一度直した科目・公私区分は次の取込で消える。消す・選ぶ・戻すを利用者の手に渡し、一度した手当てを二度させない。"
goal: "取込単位・期間単位・種別単位・全件の4粒度で消せ、実行前に巻き添えを見せ、実行後に戻せる。取込のたびの3点比較で手当てが自動的に維持され、真の衝突だけが行として提示され、取引先単位の決め事が蓄積して問われる件数が逓減する。"
scope_in: ["取込単位/期間/種別/全件の4粒度削除", "実行前 preflight と確認指紋", "D1退避テーブルによる undo と有限保持", "削除・上書きの監査記録", "取込実行前の差分プレビュー", "base/current/incoming の3点比較による手当ての継続再適用", "tx_edits への base_cls / base_owner / stable_key 追加", "取引先単位の決め事(vendor_memory)と確信度", "決め事の一覧・取消・pin・再判定"]
...
2. **上書きの判断が実装に固定されている。** 既存と新規が食い違ったときどちらを採るかを
   実装が一律に決めており、何がどう変わるかを実行前に見る手段が無い。
3. **手当てが続かない。** 一度直した科目・公私区分が次の取込で消え、同じ取引先が来るたびに
   同じ入力を繰り返す。`tx_edits` は `base_major` / `base_mid` を既に持つのに、比較に使っていない。
...
2. `balance_entries` / `cash_entries` の由来 `import_id` の持ち方が未確認。
   取込単位削除の対象特定がこの2種で成立するかが決まっていない。
