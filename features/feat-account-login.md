...
scope_in:
  ...
  - "参照画像 01-login.png を正とするログイン画面 (2カラム・安心3項目・アイコン+ラベル併記)"
...
acceptance:
  ...
  - "パスワードが復元不能な形 (PBKDF2-HMAC-SHA256 / 210,000 回以上) で保存され、平文がどこにも残らない"
  - "管理者が設定画面から利用者の追加・停止・一時パスワード再発行を完結でき、一時パスワードは発行直後の1度だけ表示される"
  - "保持 ON で30日・OFF で12時間のセッション期限となり、既定の帰結がチェックボックス脇に明示されている"
  - "ログイン画面の DOM に header/footer/サイドバーのナビゲーション要素が0件で、アイコンが常にテキストラベルと併記されている"
