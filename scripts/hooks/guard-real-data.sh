  ...
  LC_ALL=C awk -v root="$repo_root" '
    {
      ...
      if (FILENAME == root "/system-spec/spec-state.json" ||
        ...
        # premise は「個人の口座・明細そのもの」を指す語だけに限る。
        # 「実データ」「実測(値)」は一般語で、仕様方針の記述 (窓の外に実データが
        # あれば参照する) や架空フィクスチャの提示 (実測値は ¥240,000) にも出る。
        # これらを premise に数えると、同じ行に居合わせた仕様上の数量 —
        # §3.5 のフィクスチャや「その他 1 行」のような表の行数 — だけで落ちる。
        # 止めたいのは実 CSV の件数・金額であって、仕様が書けなくなる検査ではない。
        premise = "(実CSV|実明細|実入力|実ファイル|実取引|実口座)"
