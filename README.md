...
- **確定申告の準備を毎年作り直す** → 申告対象年ごとに科目・家事按分を明示確認し、freee・e-Taxへの転記補助CSVとR2原本だけの証憑ZIPを書き出す
...
| [`specs/tax-preparation.md`](specs/tax-preparation.md) | 確定申告の準備・証憑取得先の詳細lifecycle・受入の正本 |
...
5. 「領収書の残り」で未添付の事業経費を潰す（スマホで添付でき、取得先URL・サービス名・ログインに使うアカウント名・メモは同じ取引先の翌月へ引き継げる）
6. バックアップは夜間cronがR2へ自動保存（30日保持）。手動なら「設定 → 統合データJSON」

## 確定申告のときの手順

1. 「確定申告の準備」専用の対象年を選ぶ（分析用の全期間・任意期間は申告計算へ流れない）
2. 準備チェックで「要対応」を消す
3. 科目の割り当てと家事按分を確認して保存する（全額事業でも100%を明示し、按分時は根拠が必須）
4. 「領収書の残り」でMFの事業支出と事業現金の未添付を解消する。freee仕訳の証憑はfreee側で確認する
5. 「転記シート(CSV)」「科目別経費内訳(CSV)」をfreee・e-Taxへの転記補助として書き出す
6. 「証憑まとめ(ZIP)」を保存する。件数が多い場合は安全に複数ZIPへ分割され、各ZIPの索引CSVと実ファイルは同じ集合になる

証憑の取得先にはHTTP/HTTPS URLだけを保存できる。パスワード・認証トークンは保存せず、リンク先のサービス側で管理する。
この機能は「確定申告の準備」と転記の補助であり、申告書の生成や税務上の適法性を保証しない。

「領収書の残り」の緊急度は、**支払手段に代替証憑があるか**と**金額の帯**の2つだけで決めている。
3万円以上を手段によらず「要対応」にするのは、作業順を決めるためのアプリ内の目安であり、法的な要否判定ではない。
それ未満は口座履歴の残らない現金・支払手段不明を1段重く見る（`RECEIPT_MAJOR_AMOUNT` / `RECEIPT_MINOR_AMOUNT`）。科目名は判定に使わない（名前を変えた瞬間に
静かに緊急度が下がるため）。

減価償却・棚卸などの決算整理と、青色申告特別控除・所得控除はこのシステムでは計算しない。
転記シートにも同じ注意書きを必ず付けて書き出す。
