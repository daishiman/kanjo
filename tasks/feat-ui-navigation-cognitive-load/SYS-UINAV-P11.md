...
file_path: "tasks/feat-ui-navigation-cognitive-load/SYS-UINAV-P11.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-29T15:48:08Z","origin_kind":"system-dev-planner","source_digest":"be365873e506ea86341c1df37ab821656834663dd168ce6eb1aa4beb7bd13ccd","source_path":".dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/task-specs/phase-11-evidence.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
created_at: "2026-08-29T15:48:08Z"
updated_at: "2026-08-29T15:48:08Z"
depends_on: ["SYS-UINAV-P10"]
related_nodes: ["spec-ui-navigation-cognitive-load","arch-ui-navigation-experience","arch-ui-navigation-frontend"]
resource_scope: [".dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-11-evidence-index.md"]
purpose: "再現可能な検証証跡の集約の完了状態と検証可能な証跡を確定する。"
goal: "コマンド結果、画面確認、受入判定を機密情報なしで追跡可能な索引にまとめる。"
scope_in: [".dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-11-evidence-index.md"]
scope_out: ["会計計算、API、データ、認証、インフラの契約変更","commit、push、Pull Request作成、本番deploy"]
acceptance: ["コマンド結果、画面確認、受入判定を機密情報なしで追跡可能な索引にまとめる。","対象成果物が feature の受入条件と architecture の境界に一致し、実データや秘密情報を含まない。","検証結果と未解決事項が task の証跡へ記録され、高重大度の未解決指摘がない。"]
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-ui-navigation-cognitive-load/SYS-UINAV-P11.md","confidence":1}]
...
> **この文書が正本。** `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/task-specs/phase-11-evidence.md` はこれを生成したstaging snapshotで、promotion後の更新は本書だけに入れる(`source_lineage.source_digest`はpromotion時点の対応を示すもので、以後の同期は保証されない)。
...
`feat-ui-navigation-cognitive-load` の exact-13 package における P11。確定仕様、共通ナビゲーション設計、task-graph handoff を同じ source digest で参照し、現在地・情報優先度・編集安全性を一貫して改善する。
...
- 入力: `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/task-specs/phase-11-evidence.md`
- 前提: `SYS-UINAV-P10` の完了
- Source digest: `sha256:be365873e506ea86341c1df37ab821656834663dd168ce6eb1aa4beb7bd13ccd`

## 出力と成果物

- 生成または更新: `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-11-evidence-index.md`
...
- `touches`: `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-11-evidence-index.md`
- 排他資源: `SYS-UINAV-P11`
...
- PR linkage requirement: PR本文へ beads識別子と `dev-graph: SYS-UINAV-P11` を記載する
...
- 実行結果(test件数、PASS数)は `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/`
  に置く。`docs/ui-navigation-guidelines.md` は件数を持たない(testを1本足した瞬間に古くなるため)。
- 文書反映の証跡は `docs/product/elegant-review-uiux/T5-spec-reflection-receipt.md`。
- 未解決事項(グラフ投影の2件)は同受領書に記録した。
