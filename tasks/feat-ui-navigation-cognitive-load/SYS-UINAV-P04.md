...
file_path: "tasks/feat-ui-navigation-cognitive-load/SYS-UINAV-P04.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-29T15:48:08Z","origin_kind":"system-dev-planner","source_digest":"be365873e506ea86341c1df37ab821656834663dd168ce6eb1aa4beb7bd13ccd","source_path":".dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/task-specs/phase-04-test-design.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
created_at: "2026-08-29T15:48:08Z"
updated_at: "2026-08-29T15:48:08Z"
depends_on: ["SYS-UINAV-P03"]
related_nodes: ["spec-ui-navigation-cognitive-load","arch-ui-navigation-experience","arch-ui-navigation-frontend"]
resource_scope: [".dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-04-test-design.md"]
purpose: "回帰・視覚テスト設計の完了状態と検証可能な証跡を確定する。"
goal: "active 状態、アイコン、レスポンシブ、編集パネル、段階的開示を再現可能に検証するテスト設計を確定する。"
scope_in: [".dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-04-test-design.md"]
scope_out: ["会計計算、API、データ、認証、インフラの契約変更","commit、push、Pull Request作成、本番deploy"]
acceptance: ["active 状態、アイコン、レスポンシブ、編集パネル、段階的開示を再現可能に検証するテスト設計を確定する。","対象成果物が feature の受入条件と architecture の境界に一致し、実データや秘密情報を含まない。","検証結果と未解決事項が task の証跡へ記録され、高重大度の未解決指摘がない。"]
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-ui-navigation-cognitive-load/SYS-UINAV-P04.md","confidence":1}]
...
> **この文書が正本。** `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/task-specs/phase-04-test-design.md` はこれを生成したstaging snapshotで、promotion後の更新は本書だけに入れる(`source_lineage.source_digest`はpromotion時点の対応を示すもので、以後の同期は保証されない)。

## 目的

active 状態、アイコン、レスポンシブ、編集パネル、段階的開示を再現可能に検証するテスト設計を確定する。

## 背景

`feat-ui-navigation-cognitive-load` の exact-13 package における P04。確定仕様、共通ナビゲーション設計、task-graph handoff を同じ source digest で参照し、現在地・情報優先度・編集安全性を一貫して改善する。
...
- 補足: 構造契約をDOM testへ落とす。visualはP07/P09が担う
- 充足状況の最新はfeature `features/feat-ui-navigation-cognitive-load.md` の受入節を見る。

## 入力と前提条件

- 入力: `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/task-specs/phase-04-test-design.md`
- 前提: `SYS-UINAV-P03` の完了
- Source digest: `sha256:be365873e506ea86341c1df37ab821656834663dd168ce6eb1aa4beb7bd13ccd`

## 出力と成果物

- 生成または更新: `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-04-test-design.md`
...
- `touches`: `.dev-graph/plans/feature-package-feat-ui-navigation-cognitive-load/evidence/phase-04-test-design.md`
- 排他資源: `SYS-UINAV-P04`
...
- PR linkage requirement: PR本文へ beads識別子と `dev-graph: SYS-UINAV-P04` を記載する
...
- **CSSの正規表現照合を全廃した。** 描画を一切見ていないため、通っても視覚契約の証拠にならない。
  代わりに実Chrome(CDP)で実際にレンダリングし、`getBoundingClientRect` / `getComputedStyle` から
  数値を取る方式へ移行した。スクリーンショットの目視より再現性が高い。
- Chrome の起動処理は `scripts/headless-chrome.mjs` へ集約し、実描画検査3スクリプトで共有する。
