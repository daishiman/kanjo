---
graph_node_id: "SYS-AI-P03"
artifact_kind: "task"
artifact_subtypes: []
title: "段階契約・API 契約・データ公開範囲・URL 契約の独立レビュー"
...
iteration: null
owners: ["daishiman"]
tags: ["ai-analysis", "p03", "design-review"]
file_path: "tasks/feat-ai-analysis-screen/sys-ai-p03.md"
template_id: "task"
...
source_lineage: {"imported_at": "2026-09-19T13:50:42Z", "origin_kind": "system-dev-planner", "source_digest": "dede168b82fcdec85ff00d13378d586344ebdd90747d5ea9bd779c6a825d5405", "source_path": ".dev-graph/plans/feature-package-feat-ai-analysis-screen/task-specs/phase-03-design-review.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
created_at: "2026-09-19T13:50:42Z"
updated_at: "2026-09-19T13:50:42Z"
depends_on: ["SYS-AI-P02"]
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-ai-analysis-screen/sys-ai-p03.md", "confidence": 0.95}]
...
# 段階契約・API 契約・データ公開範囲・URL 契約の独立レビュー
...
- parent_feature: feat-ai-analysis-screen
- phase_ref: P03
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-ai-analysis-screen/sys-ai-p03.md
...
P02 の設計決定を、段階の境界・トークン失効・body 上限・外へ出るデータの範囲・URL 復元の観点で独立にレビューし、指摘を決定記録へ反映する。

## 背景

body 上限は spec の 4 MiB と security 章の 256KB が食い違っている (OI-02)。キャンセル済みトークンの 401、集計値だけを出す規則、既存行の非書き換えは、実装前に設計の段階で矛盾を潰す。

## 前提条件

- Required spec/architecture/phase/task nodes: arch-ai-analysis-auth, arch-ai-analysis-backend, arch-ai-analysis-database, arch-ai-analysis-frontend, arch-ai-analysis-infrastructure, arch-ai-analysis-maintenance-ops, arch-ai-analysis-security, arch-ai-analysis-ui-ux, spec-ai-analysis-screen, SYS-AI-P02
...
- Frontend: applicable: URL 復元と状態の扱いをレビューする
- Backend: applicable: 段階導出の境界をレビューする
- API: N/A: 本 phase では扱わない
- Data: N/A: 本 phase では扱わない
- Infrastructure: N/A: 基盤は変更しない
- Security: applicable: トークン失効・body 上限・公開範囲をレビューする
- Quality: applicable: レビュー指摘を記録する
- Documentation: applicable: 指摘の反映を docs に記録する
...
- PR body contract: dev-graph graph_node_id SYS-AI-P03 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-AI-P03 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-AI-P03 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-AI-P02) が完了し、write_scope が他の active lease と重複しないこと
...
- Acceptance:
- OI-02 (body 上限の食い違い) の結論が決定記録にある。
- 段階の境界 4 件の期待値がレビュー済みとして記録されている。
- 外へ出るデータが集計値だけであることの検査方法が決まっている。
- Automated commands:
- pnpm lint
- Required evidence:
- docs/ai-screen/design-decisions.md
