...
artifact_kind: "task"
artifact_subtypes: []
title: "cash-screen 純関数・論理削除と復元の 4 経路・migration 0052・夜間消去・画面分割のワークストリーム設計決定記録"
...
# cash-screen 純関数・論理削除と復元の 4 経路・migration 0052・夜間消去・画面分割のワークストリーム設計決定記録
...
core の cash-screen 純関数の入出力、論理削除・復元・一括の経路、読取経路 5 本の条件、migration 0052、夜間の完全消去、画面の分割単位を 1 つの決定記録にまとめる。
...
- Data: applicable: 0052 の列と索引、読取経路 5 本の条件を決める
...
- Compatibility/migration/backfill: N/A: 設計のみ。0052 は追加のみで既存行の更新 0 件であることを設計として明記する
...
- migration 0052 が追加のみで既存行の更新 0 件であり、夜間の完全消去が 1 晩 500 行で tx_edits も同じ batch で消すことが設計として明記されている。
- Automated commands:
- pnpm lint
- Required evidence:
- docs/cash-screen/design-decisions.md
