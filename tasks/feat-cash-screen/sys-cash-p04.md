...
artifact_kind: "task"
artifact_subtypes: []
title: "core 境界値・API 契約・migration 0050・夜間予算・DOM の失敗テスト先行作成"
...
resource_scope: ["packages/core/test/cash-screen.test.ts", "packages/api/src/cash-screen.integration.test.ts", "packages/api/src/cash-migration-0050.test.ts", "packages/web/src/pages/cash/cash-screen.dom.test.tsx", "packages/api/src/scheduled-maintenance-budget.test.ts", "packages/api/src/import-lifecycle-pure.test.ts", "packages/api/src/schema-guard.test.ts", "packages/api/src/import-lifecycle.test.ts"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
# core 境界値・API 契約・migration 0050・夜間予算・DOM の失敗テスト先行作成
...
- Data: applicable: migration 0050 のテストを書く
...
- 他人の id に 404、不正な入力に 400、migration 0050 の適用で既存行の更新 0 件のテストがある。
