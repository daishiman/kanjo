---
acceptance: ["クライアント側マスクを外した診断情報を直接投稿しても、保存後の値から Cookie と Authorization と Bearer とパスワード相当の値が除去されている", "TTL 超過のトークンと取得回数上限超過のトークンが、互いに区別できる拒否理由で拒否され、いずれも 500 にならない", "改善要望のテーブル名が packages/api/src/store.ts の BACKUP_SNAPSHOT_SQL に1つも現れないことを検証が固定している", "対応完了から30日を超えた要望の添付が削除され、本文と状態と対応記録が参照できることを検証が固定している"]
architecture_refs: ["arch-improvement-request-pipeline"]
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: {"bd_issue_id": "kanjo-4o6", "github_mirror": null, "linked_at": "2026-08-30T13:04:20Z", "sync_state": "synced"}
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-improvement-request/SYS-IMPREQ-P09.md", "confidence": 1}]
...
depends_on: ["SYS-IMPREQ-P08"]
domain: "security"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-improvement-request"
file_path: "tasks/feat-improvement-request/SYS-IMPREQ-P09.md"
github_project_linkages: []
github_publication: {"labels": [], "milestone": null, "mode": "local_only", "project_aliases": []}
goal: "秘匿値が保存とログへ到達しないこと、および30日で添付だけが消えることを、退行しない形で固定する。"
...
purpose: "秘匿値と保持期間の品質保証の完了状態と検証可能な証跡を確定する。"
related_nodes: ["arch-improvement-request-pipeline"]
resource_scope: ["packages/api/src/improvement-redaction.test.ts", "packages/api/src/improvement-retention.test.ts", "packages/api/src/improvement-backup-exclusion.test.ts"]
scope_in: ["packages/api/src/improvement-redaction.test.ts", "packages/api/src/improvement-retention.test.ts", "packages/api/src/improvement-backup-exclusion.test.ts"]
...
source_lineage: {"imported_at": "2026-08-30T12:47:55Z", "origin_kind": "system-dev-planner", "source_digest": "e5aaef0b452885b06b9210c427c16a97c47e85a0a1c9716a17e183e53f08cf07", "source_path": ".dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-09-quality-assurance.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
start_date: null
status: "active"
...
> **この文書が正本。** `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-09-quality-assurance.md` はこれを生成した staging snapshot で、promotion 後の更新は本書だけに入れる(`source_lineage.source_digest` は promotion 時点の対応を示すもので、以後の同期は保証されない)。
...
- 前提: P08 SYS-IMPREQ-P08 が完了していること
...
- 入力: `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-09-quality-assurance.md`
...
- `touches`: packages/api/src/improvement-redaction.test.ts、packages/api/src/improvement-retention.test.ts、packages/api/src/improvement-backup-exclusion.test.ts
- 排他資源: `SYS-IMPREQ-P09`
...
- PR linkage requirement: PR 本文へ beads 識別子と `dev-graph: SYS-IMPREQ-P09` を記載する
...
- 検証: マスクを外した入力を投稿する検証を実行し、保存値に秘匿値が残らないことを確認する
- 検証: 期限超過と回数超過をそれぞれ再現し、応答の拒否理由が区別できることを確認する
