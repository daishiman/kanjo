---
acceptance: ["features/feat-improvement-request.md の受入条件15件が漏れなくベースラインへ写され、各件に検証可能な観測点が付いている", "撮影・診断・受け渡し・失効の4層それぞれについて、実装が触れてよい既存資産と触れてはならない既存資産が列挙されている", "本番 D1 のスキーマ版数が不足しており migration を本番へ配信できないという前提が、ベースラインに明記されている"]
architecture_refs: ["arch-improvement-request-pipeline"]
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: {"bd_issue_id": "kanjo-98s", "github_mirror": null, "linked_at": "2026-08-30T13:04:20Z", "sync_state": "synced"}
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-improvement-request/SYS-IMPREQ-P01.md", "confidence": 1}]
...
file_path: "tasks/feat-improvement-request/SYS-IMPREQ-P01.md"
github_project_linkages: []
github_publication: {"labels": [], "milestone": null, "mode": "local_only", "project_aliases": []}
goal: "確定仕様の受入15件を、実装が参照できる粒度の要件ベースラインへ写し取り、参照先の digest とともに固定する。"
graph_node_id: "SYS-IMPREQ-P01"
...
purpose: "改善要望の実行要件ベースライン確定の完了状態と検証可能な証跡を確定する。"
related_nodes: ["arch-improvement-request-pipeline"]
resource_scope: [".dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-01-requirements-baseline.json"]
scope_in: [".dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-01-requirements-baseline.json"]
...
source_lineage: {"imported_at": "2026-08-30T12:47:55Z", "origin_kind": "system-dev-planner", "source_digest": "e5aaef0b452885b06b9210c427c16a97c47e85a0a1c9716a17e183e53f08cf07", "source_path": ".dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-01-requirements.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
start_date: null
status: "active"
...
> **この文書が正本。** `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-01-requirements.md` はこれを生成した staging snapshot で、promotion 後の更新は本書だけに入れる(`source_lineage.source_digest` は promotion 時点の対応を示すもので、以後の同期は保証されない)。

## 目的

確定仕様の受入15件を、実装が参照できる粒度の要件ベースラインへ写し取り、参照先の digest とともに固定する。
...
- features/feat-improvement-request.md の受入条件15件が漏れなくベースラインへ写され、各件に検証可能な観測点が付いている
- 撮影・診断・受け渡し・失効の4層それぞれについて、実装が触れてよい既存資産と触れてはならない既存資産が列挙されている
- 本番 D1 のスキーマ版数が不足しており migration を本番へ配信できないという前提が、ベースラインに明記されている
- 充足状況の最新は feature `features/feat-improvement-request.md` の受入節を見る。

## 入力と前提条件

- 前提: 先行 phase は無い。features/feat-improvement-request.md が confirmed であることが起点となる
...
- 入力: `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-01-requirements.md`
...
- Produced artifacts: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-01-requirements-baseline.json
...
- `touches`: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-01-requirements-baseline.json
- 排他資源: `SYS-IMPREQ-P01`
...
- PR linkage requirement: PR 本文へ beads 識別子と `dev-graph: SYS-IMPREQ-P01` を記載する
...
- 検証: 受入条件15件とベースライン項目の対応表を作り、対応先が空の行が0件であることを確認する
- 検証: 参照した system-spec と architecture の digest がベースラインに記録され、現行ファイルの digest と一致することを確認する
- Required evidence: P01 の受入条件、実行結果、差分対象、未解決指摘の有無を記録する
- Privacy: 画面証跡は合成または既存の匿名表示のみとし、口座明細や金額の実データを含めない

## リスクとロールバック

- Rollout: 依存 phase の完了後に write scope 内だけを変更し、ローカルとテスト環境で確認する
- Rollback trigger and steps: ベースラインは読み取りと記録だけで既存コードを変えないため、生成した証跡ファイルを削除すれば元に戻る。
...
- Dependency: 先行 phase 無し
- Task graph: task-graph.json の P01 node
