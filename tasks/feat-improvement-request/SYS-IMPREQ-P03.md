---
acceptance: ["トークン値・Cookie・Authorization ヘッダが保存先とログのどこにも到達しないことが、経路単位で追跡され記録されている", "30日削除が複製経路によって骨抜きにならないことが、バックアップ・エクスポート・詳細取得の各経路について確認されている", "高重大度の未解決指摘が0件である"]
architecture_refs: ["arch-improvement-request-pipeline"]
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: {"bd_issue_id": "kanjo-7yq", "github_mirror": null, "linked_at": "2026-08-30T13:04:20Z", "sync_state": "synced"}
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-improvement-request/SYS-IMPREQ-P03.md", "confidence": 1}]
...
depends_on: ["SYS-IMPREQ-P02"]
domain: "quality"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-improvement-request"
file_path: "tasks/feat-improvement-request/SYS-IMPREQ-P03.md"
github_project_linkages: []
github_publication: {"labels": [], "milestone": null, "mode": "local_only", "project_aliases": []}
goal: "P02 の契約を実装着手前に独立の観点でレビューし、秘匿値の漏えい経路と保持期間の抜け穴を先に潰す。"
graph_node_id: "SYS-IMPREQ-P03"
...
purpose: "境界設計の独立レビューの完了状態と検証可能な証跡を確定する。"
related_nodes: ["arch-improvement-request-pipeline"]
resource_scope: [".dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-03-design-review.json"]
scope_in: [".dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-03-design-review.json"]
...
source_lineage: {"imported_at": "2026-08-30T12:47:55Z", "origin_kind": "system-dev-planner", "source_digest": "e5aaef0b452885b06b9210c427c16a97c47e85a0a1c9716a17e183e53f08cf07", "source_path": ".dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-03-design-review.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
start_date: null
status: "active"
...
> **この文書が正本。** `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-03-design-review.md` はこれを生成した staging snapshot で、promotion 後の更新は本書だけに入れる(`source_lineage.source_digest` は promotion 時点の対応を示すもので、以後の同期は保証されない)。

## 目的

P02 の契約を実装着手前に独立の観点でレビューし、秘匿値の漏えい経路と保持期間の抜け穴を先に潰す。
...
- トークン値・Cookie・Authorization ヘッダが保存先とログのどこにも到達しないことが、経路単位で追跡され記録されている
- 30日削除が複製経路によって骨抜きにならないことが、バックアップ・エクスポート・詳細取得の各経路について確認されている
- 高重大度の未解決指摘が0件である
- 充足状況の最新は feature `features/feat-improvement-request.md` の受入節を見る。

## 入力と前提条件

- 前提: P02 SYS-IMPREQ-P02 が完了していること
...
- 入力: `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-03-design-review.md`
...
- Produced artifacts: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-03-design-review.json
...
- `touches`: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-03-design-review.json
- 排他資源: `SYS-IMPREQ-P03`
...
- PR linkage requirement: PR 本文へ beads 識別子と `dev-graph: SYS-IMPREQ-P03` を記載する
...
- 検証: 秘匿値の流れをクライアント収集からサーバー保存とログ出力まで経路単位で追跡し、マスクが二重に掛かることを確認する
- 検証: P02 の契約と features/feat-improvement-request.md の scope_out 8件を突き合わせ、逸脱が0件であることを確認する
- Required evidence: P03 の受入条件、実行結果、差分対象、未解決指摘の有無を記録する
- Privacy: 画面証跡は合成または既存の匿名表示のみとし、口座明細や金額の実データを含めない

## リスクとロールバック

- Rollout: 依存 phase の完了後に write scope 内だけを変更し、ローカルとテスト環境で確認する
- Rollback trigger and steps: レビュー記録のみの変更のため、証跡ファイルを削除すれば元に戻る。
...
- Dependency: SYS-IMPREQ-P02
- Task graph: task-graph.json の P03 node
