---
acceptance: ["受入条件15件のすべてに、対応する自動検証が少なくとも1件割り当てられている", "撮影順序・二重マスク・トークンのハッシュ保存・拒否理由の区別・バックアップ非追加の5点が、それぞれ独立した検証として設計されている", "実データと秘密情報を用いない検証データの作り方が設計に含まれている"]
architecture_refs: ["arch-improvement-request-pipeline"]
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: {"bd_issue_id": "kanjo-8cw", "github_mirror": null, "linked_at": "2026-08-30T13:04:20Z", "sync_state": "synced"}
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-improvement-request/SYS-IMPREQ-P04.md", "confidence": 1}]
...
depends_on: ["SYS-IMPREQ-P03"]
domain: "quality"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-improvement-request"
file_path: "tasks/feat-improvement-request/SYS-IMPREQ-P04.md"
github_project_linkages: []
github_publication: {"labels": [], "milestone": null, "mode": "local_only", "project_aliases": []}
goal: "受入15件それぞれを、自動実行できる検証へ機械的に写像し、どの検証がどの受入条件を守るかを固定する。"
graph_node_id: "SYS-IMPREQ-P04"
...
purpose: "受入条件を検証へ写す試験設計の完了状態と検証可能な証跡を確定する。"
related_nodes: ["arch-improvement-request-pipeline"]
resource_scope: [".dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-04-test-design.json"]
scope_in: [".dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-04-test-design.json"]
...
source_lineage: {"imported_at": "2026-08-30T12:47:55Z", "origin_kind": "system-dev-planner", "source_digest": "e5aaef0b452885b06b9210c427c16a97c47e85a0a1c9716a17e183e53f08cf07", "source_path": ".dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-04-test-design.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
start_date: null
status: "active"
...
> **この文書が正本。** `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-04-test-design.md` はこれを生成した staging snapshot で、promotion 後の更新は本書だけに入れる(`source_lineage.source_digest` は promotion 時点の対応を示すもので、以後の同期は保証されない)。

## 目的

受入15件それぞれを、自動実行できる検証へ機械的に写像し、どの検証がどの受入条件を守るかを固定する。
...
- 受入条件15件のすべてに、対応する自動検証が少なくとも1件割り当てられている
- 撮影順序・二重マスク・トークンのハッシュ保存・拒否理由の区別・バックアップ非追加の5点が、それぞれ独立した検証として設計されている
- 実データと秘密情報を用いない検証データの作り方が設計に含まれている
- 充足状況の最新は feature `features/feat-improvement-request.md` の受入節を見る。

## 入力と前提条件

- 前提: P03 SYS-IMPREQ-P03 が完了していること
...
- 入力: `.dev-graph/plans/feature-package-feat-improvement-request/task-specs/phase-04-test-design.md`
...
- Produced artifacts: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-04-test-design.json
...
- `touches`: .dev-graph/plans/feature-package-feat-improvement-request/evidence/phase-04-test-design.json
- 排他資源: `SYS-IMPREQ-P04`
...
- PR linkage requirement: PR 本文へ beads 識別子と `dev-graph: SYS-IMPREQ-P04` を記載する
...
- 検証: 受入条件と検証の対応表を作り、検証が割り当てられていない受入条件が0件であることを確認する
- 検証: 設計した検証が既存の vitest 構成 (packages/web は jsdom、packages/api は Workers) の上で実行可能な形であることを確認する
- Required evidence: P04 の受入条件、実行結果、差分対象、未解決指摘の有無を記録する
- Privacy: 画面証跡は合成または既存の匿名表示のみとし、口座明細や金額の実データを含めない

## リスクとロールバック

- Rollout: 依存 phase の完了後に write scope 内だけを変更し、ローカルとテスト環境で確認する
- Rollback trigger and steps: 試験設計のみの変更のため、証跡ファイルを削除または前版へ戻す。
...
- Dependency: SYS-IMPREQ-P03
- Task graph: task-graph.json の P04 node
