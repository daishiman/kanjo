...
artifact_kind: "task"
artifact_subtypes: []
title: "import-screen 純関数・検査と確定の API・migration 0050・データ取込画面の分割と旧操作の移設の最終実装"
...
# import-screen 純関数・検査と確定の API・migration 0050・データ取込画面の分割と旧操作の移設の最終実装
...
- Data: applicable: migration 0050 と schema.ts と runtimeSchemaGuard を揃える
...
- Deploy unit/environment: web ビルドと Worker と D1 migration (0050 の表と列の追加のみ)
...
- migration 0050 が追加のみで runtimeSchemaGuard の必須列に含まれる。
...
- Rollback trigger and steps: 本 task のコミットを revert する。0050 は表と列の追加のみで既存行を書き換えないため、表と列が残っても旧画面は動く。配信済みなら直前のビルドへ戻し、表と列の削除は行わない。
