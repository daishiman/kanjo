...
本 feature の変更を単一 PR で配信し、Migrate → Deploy の順で 0053 を適用して、完了を記録する。

## 背景

0053 は表と列の追加のみで既存行を書き換えないため、Migrate を先に行っても旧 Worker は動く。配信後に既存行の更新 0 件と、夜間保守の片づけが動くことを確かめる。
...
- Data: applicable: migration 0053 を適用する
...
- Compatibility/migration/backfill: 0053 は追加のみ。表と列は削除しない
...
- Rollback trigger and steps: PR を revert して直前のビルドへ戻す。0053 は追加のみの表と列なので削除しない。
