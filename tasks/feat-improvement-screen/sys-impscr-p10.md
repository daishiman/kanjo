...
iteration: null
owners: ["daishiman"]
tags: ["improvement", "p10", "independent-review"]
file_path: "tasks/feat-improvement-screen/sys-impscr-p10.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-improvement-screen/sys-impscr-p10.md", "confidence": 0.95}]
...
- parent_feature: feat-improvement-screen
- phase_ref: P10
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-improvement-screen/sys-impscr-p10.md
...
P01〜P09 の記録と差分を独立に読み、配信可否を判断する。

## 背景

0053 は表の作り直しを伴い巻き戻しが対称でない。配信可否の判断に巻き戻しの前提を含める。
...
- Frontend: N/A: レビュー記録のみ
- Backend: N/A: レビュー記録のみ
- API: N/A: レビュー記録のみ
- Data: N/A: レビュー記録のみ
- Infrastructure: N/A: レビュー記録のみ
- Security: N/A: レビュー記録のみ
- Quality: applicable: 配信可否を判断する
- Documentation: N/A: レビュー記録のみ
- Operations: N/A: レビュー記録のみ
...
- PR body contract: dev-graph graph_node_id SYS-IMPSCR-P10 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-IMPSCR-P10 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-IMPSCR-P10 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-IMPSCR-P09) が完了し、write_scope が他の active lease と重複しないこと
...
- Acceptance:
- 配信可否の判断と根拠が記録されている。
- 巻き戻しの前提 (再確認の状態と削除中の行の有無の確認) が判断に含まれている。
- Automated commands:
- pnpm test
- pnpm typecheck
- Required evidence:
