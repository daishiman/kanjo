---
graph_node_id: "SYS-IMPSCR-P03"
artifact_kind: "task"
artifact_subtypes: []
title: "削除と復元の契約・読取経路・マスクの 2 境界・入力検証・URL 契約の独立レビュー"
...
iteration: null
owners: ["daishiman"]
tags: ["improvement", "p03", "design-review"]
file_path: "tasks/feat-improvement-screen/sys-impscr-p03.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-improvement-screen/sys-impscr-p03.md", "confidence": 0.95}]
...
# 削除と復元の契約・読取経路・マスクの 2 境界・入力検証・URL 契約の独立レビュー
...
- parent_feature: feat-improvement-screen
- phase_ref: P03
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-improvement-screen/sys-impscr-p03.md
...
P02 の決定を現物のコードと突き合わせてレビューし、P04 の失敗テストの期待値を固定できる状態にする。

## 背景

improvement_requests を読む経路は routes/improvement.ts・improvement/・store.ts・孤児の掃除に散っている。1 経路でも削除中の行の除外が漏れると、削除した依頼が一覧や agent 経路に現れる。agentGuard は行が無いとき 401 を返し、章は 404 と決めている (OI-06)。
...
- Frontend: N/A: レビューのみ
- Backend: applicable: 読取経路と削除・復元の契約をレビューする
- API: N/A: レビューのみ
- Data: N/A: レビューのみ
- Infrastructure: N/A: レビューのみ
- Security: applicable: 他人の依頼の 404・マスクの 2 境界・agentGuard の応答をレビューする
- Quality: applicable: テストの期待値を固定する
- Documentation: N/A: レビューのみ
- Operations: N/A: レビューのみ
...
- PR body contract: dev-graph graph_node_id SYS-IMPSCR-P03 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-IMPSCR-P03 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-IMPSCR-P03 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-IMPSCR-P02) が完了し、write_scope が他の active lease と重複しないこと
...
- Acceptance:
- improvement_requests を読む経路の一覧が現物の grep と一致することがレビュー済みとして記録されている。
- OI-06 (削除中のトークンと agentGuard の 401 / 404) の結論が決定記録にある。
- 他人の依頼の取得・変更・削除・復元が 404、不正な入力が 400、許されない遷移の応答の期待値がレビュー済みである。
- Automated commands:
- pnpm lint
- Required evidence:
- docs/improvement-screen/design-decisions.md
