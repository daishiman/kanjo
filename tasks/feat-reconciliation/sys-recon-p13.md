---
graph_node_id: "SYS-RECON-P13"
artifact_kind: "task"
artifact_subtypes: []
title: "単一PRでの配信とクローズアウト"
...
iteration: null
owners: ["daishiman"]
tags: ["reconciliation", "p13", "release-deploy"]
file_path: "tasks/feat-reconciliation/sys-recon-p13.md"
template_id: "task"
...
source_lineage: {"imported_at": "2026-09-15T11:23:54Z", "origin_kind": "system-dev-planner", "source_digest": "76c705dfa24c67d6b3bf7e3e42d55f2e7b60c0a316f4df01b0343abd909431ad", "source_path": ".dev-graph/plans/feature-package-feat-reconciliation/task-specs/phase-13-release-deploy.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
created_at: "2026-09-15T11:23:54Z"
updated_at: "2026-09-15T11:23:54Z"
depends_on: ["SYS-RECON-P12"]
...
resource_scope: ["docs/reconciliation/close-out.md"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-reconciliation/sys-recon-p13.md", "confidence": 0.95}]
...
- parent_feature: feat-reconciliation
- phase_ref: P13
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-reconciliation/sys-recon-p13.md
...
単一PRでのリリース経路を確認し、rollback手順を記録してfeatureをクローズアウトする。

## 背景

architecture/reconciliation-infrastructure.mdは追加のみのmigrationがdeploy.ymlの自動適用経路に乗ることを推定 (qa-infrastructure-web-rc-inference-002) しており、P13でその経路を実測確認する。
...
- Depends on: SYS-RECON-P12 の成果物が確定していること

## Workstream applicability

- Frontend: N/A: 本taskはビルド成果物の生成確認のみ
- Backend: N/A: 本taskの関心外
- API: N/A: 本taskの関心外
- Data: N/A: 本taskの関心外
- Infrastructure: applicable: deploy.ymlの自動適用経路を確認する
- Security: N/A: 本taskの関心外
- Quality: applicable: 配信前の最終ビルド確認を行う
- Documentation: applicable: docs/reconciliation/close-out.mdを新設する
- Operations: applicable: 配信とrollback手順の確定そのもの
...
- Compatibility/migration/backfill: migrations/0040相当がdeploy.ymlの自動適用経路 (追加のみのD1 migration) に乗ることを確認する

## 成果物

- Produced artifacts:
- docs/reconciliation/close-out.md
- Consumed artifacts:
...
- PR body contract: dev-graph graph_node_id SYS-RECON-P13 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-RECON-P13 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-RECON-P13 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-RECON-P12) が完了し、write_scope が他の active lease と重複しないこと
...
- Rollback trigger and steps: Workerを直前のビルドへ戻す。D1にrollbackは無いため、mf_tx_exclusions・reconciliation_actions・monthly_reviewsは無害なまま残置する。単一PRのため1回のWorker切り戻しで完全に戻せる。
