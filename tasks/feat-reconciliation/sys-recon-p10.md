...
iteration: null
owners: ["daishiman"]
tags: ["reconciliation", "p10", "final-review"]
file_path: "tasks/feat-reconciliation/sys-recon-p10.md"
template_id: "task"
...
source_lineage: {"imported_at": "2026-09-15T11:23:54Z", "origin_kind": "system-dev-planner", "source_digest": "76c705dfa24c67d6b3bf7e3e42d55f2e7b60c0a316f4df01b0343abd909431ad", "source_path": ".dev-graph/plans/feature-package-feat-reconciliation/task-specs/phase-10-final-review.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
created_at: "2026-09-15T11:23:54Z"
updated_at: "2026-09-15T11:23:54Z"
depends_on: ["SYS-RECON-P09"]
...
resource_scope: ["docs/reconciliation/final-review.md"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-reconciliation/sys-recon-p10.md", "confidence": 0.95}]
...
- parent_feature: feat-reconciliation
- phase_ref: P10
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-reconciliation/sys-recon-p10.md
...
P01..P09の全成果物を対象に、実装者から独立した最終レビューを行い是正指摘をゼロにする。

## 背景

feature-execution-package-contractのP10は独立レビューを担う。P03の設計レビューとは異なり、実装後の全体整合を対象にする。
...
- Depends on: SYS-RECON-P09 の成果物が確定していること

## Workstream applicability

- Frontend: applicable: 画面実装のレビューを行う
- Backend: applicable: core/API実装のレビューを行う
- API: applicable: API契約実装のレビューを行う
- Data: applicable: D1新表実装のレビューを行う
- Infrastructure: N/A: 本taskの関心外
- Security: applicable: 認証・排他実装のレビューを行う
- Quality: applicable: 最終レビューそのもの
- Documentation: applicable: docs/reconciliation/final-review.mdを新設する
...
- PR body contract: dev-graph graph_node_id SYS-RECON-P10 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-RECON-P10 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-RECON-P10 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-RECON-P09) が完了し、write_scope が他の active lease と重複しないこと
...
- Rollback trigger and steps: docs/reconciliation/final-review.md の追加コミットを revert する。
