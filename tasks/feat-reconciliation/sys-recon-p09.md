---
graph_node_id: "SYS-RECON-P09"
artifact_kind: "task"
artifact_subtypes: []
title: "アクセシビリティ・入力検証・JS予算・route-icon-distinctの保証確認"
...
iteration: null
owners: ["daishiman"]
tags: ["reconciliation", "p09", "quality-assurance"]
file_path: "tasks/feat-reconciliation/sys-recon-p09.md"
template_id: "task"
...
resource_scope: ["docs/reconciliation/assurance.md"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-reconciliation/sys-recon-p09.md", "confidence": 0.95}]
...
# アクセシビリティ・入力検証・JS予算・route-icon-distinctの保証確認
...
- parent_feature: feat-reconciliation
- phase_ref: P09
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-reconciliation/sys-recon-p09.md
...
S6 (狭幅での縦積み・test/typecheck/lint・check系の緑) と入力検証・アイコン重複検査の保証を確認する。

## 背景

architecture/reconciliation-security.mdは入力検証の許可リストとcanonicalMutationFence対象化をG3の達成条件としており、architecture/reconciliation-maintenance-ops.mdはroute-icon-distinctの維持をG5としている。
...
- Depends on: SYS-RECON-P08 の成果物が確定していること

## Workstream applicability

- Frontend: applicable: 狭幅レイアウトとJS予算を検証する
- Backend: N/A: 本taskはコード変更を伴わない
- API: applicable: 入力検証400/409を検証する
- Data: N/A: 本taskの関心外
- Infrastructure: N/A: 本taskの関心外
- Security: applicable: 許可リスト検証とcanonicalMutationFenceを検証する
- Quality: applicable: check系の保証確認そのもの
- Documentation: applicable: docs/reconciliation/assurance.mdを新設する
- Operations: applicable: 運用継続の前提となる保証確認を行う
...
- PR body contract: dev-graph graph_node_id SYS-RECON-P09 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-RECON-P09 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-RECON-P09 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-RECON-P08) が完了し、write_scope が他の active lease と重複しないこと
...
- Rollback trigger and steps: docs/reconciliation/assurance.md の追加コミットを revert する。保証確認のみのためコード側の巻き戻しは不要。
