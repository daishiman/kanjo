---
graph_node_id: "SYS-RECON-P07"
artifact_kind: "task"
artifact_subtypes: []
title: "AC-001..AC-006・S1..S6受入検証"
...
iteration: null
owners: ["daishiman"]
tags: ["reconciliation", "p07", "acceptance"]
file_path: "tasks/feat-reconciliation/sys-recon-p07.md"
template_id: "task"
...
source_lineage: {"imported_at": "2026-09-15T11:23:54Z", "origin_kind": "system-dev-planner", "source_digest": "76c705dfa24c67d6b3bf7e3e42d55f2e7b60c0a316f4df01b0343abd909431ad", "source_path": ".dev-graph/plans/feature-package-feat-reconciliation/task-specs/phase-07-acceptance.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
created_at: "2026-09-15T11:23:54Z"
updated_at: "2026-09-15T11:23:54Z"
depends_on: ["SYS-RECON-P06"]
...
resource_scope: ["docs/reconciliation/acceptance.md"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-reconciliation/sys-recon-p07.md", "confidence": 0.95}]
...
- parent_feature: feat-reconciliation
- phase_ref: P07
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-reconciliation/sys-recon-p07.md
...
specs/spec-reconciliation.mdのAC-001..AC-006とgoal-spec.jsonのS1..S6を実測結果で受入検証する。

## 背景

P05の実装成果物に対し、仕様のacceptance criteriaとgoal-specのacceptanceの双方を突き合わせて検証する。
...
- Depends on: SYS-RECON-P06 の成果物が確定していること

## Workstream applicability

- Frontend: applicable: 画面描画のAC検証を行う
- Backend: applicable: core判定規則のAC検証を行う
- API: applicable: API契約のAC検証を行う
- Data: N/A: 本taskはデータ設計を変更しない
- Infrastructure: N/A: 本taskの関心外
- Security: applicable: 認証・排他のAC検証を行う
- Quality: applicable: 受入検証そのもの
- Documentation: applicable: docs/reconciliation/acceptance.mdを新設する
...
- PR body contract: dev-graph graph_node_id SYS-RECON-P07 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-RECON-P07 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-RECON-P07 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-RECON-P06) が完了し、write_scope が他の active lease と重複しないこと
...
- Rollback trigger and steps: docs/reconciliation/acceptance.md の追加コミットを revert する。
