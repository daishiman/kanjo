---
graph_node_id: "SYS-MATRIX-P04"
artifact_kind: "task"
artifact_subtypes: []
title: "core 境界値・API 契約・DOM の失敗テスト先行作成 (持ち越し4件を契約テストで固定)"
...
file_path: "tasks/feat-expense-matrix/sys-matrix-p04.md"
template_id: "task"
...
source_lineage: {"imported_at": "2026-09-16T12:34:56Z", "origin_kind": "system-dev-planner", "source_digest": "8cde5b2e1bdebb3896de7bfc985c34151547a3dd94e4884af66febbb49090a4c", "source_path": ".dev-graph/plans/feature-package-feat-expense-matrix/task-specs/phase-04-test-design.md", "source_plugin": "system-dev-planner", "source_version": "0.1.11"}
created_at: "2026-09-16T12:34:56Z"
updated_at: "2026-09-16T12:34:56Z"
depends_on: ["SYS-MATRIX-P03"]
...
resource_scope: ["packages/api/test/matrix-routes.test.ts", "packages/core/test/matrix-contract.test.ts", "packages/web/src/components/heatmap/heat-grid.dom.test.tsx", "packages/web/src/components/heatmap/heat-model.test.ts", "packages/web/src/figure-guides.test.ts", "packages/web/src/heatmap-behavior.dom.test.tsx", "packages/web/src/matrix-cell.dom.test.tsx", "packages/web/src/matrix-legend.dom.test.tsx", "packages/web/src/matrix-visual.dom.test.tsx"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-expense-matrix/sys-matrix-p04.md", "confidence": 0.95}]
...
# core 境界値・API 契約・DOM の失敗テスト先行作成 (持ち越し4件を契約テストで固定)
...
- `packages/web/src/figure-guides.test.ts` に `readFileSync` が残っていないこと。現状はモジュール最上位でファイルを読んでおり、説明文データの型が変わっても実行時まで検出できない。型で結ばれた検査に置き換える。
- 濃さの階級を決める経路に境界値テストが存在すること。`packages/api/src/ai/catalog.test.ts` には `bucketize` の境界値テストがあるが、濃淡の経路には `heatmap` / `accountMonth` を含むテストが 0 件である。
...
- 先行して固定したのは core の境界値と DOM の 2 系統。`packages/core/src/matrix-derived.test.ts` が同点解消の順序 (スコア → 金額 → 月 → 行)、窓外の前年同月、未記帳月の除外を固定している。
- DOM は `packages/web/src/components/heatmap/heat-grid.dom.test.tsx`、`heatmap-behavior.dom.test.tsx`、`packages/web/src/pages/analysis/matrix/model.test.ts` が担う。
- **API 契約テストは作っていない**。`packages/api/` に差分が無く、契約そのものが本サイクルで変わっていないため。
