---
graph_node_id: "SYS-TCSCREEN-P13"
artifact_kind: "task"
artifact_subtypes: []
title: "単一 PR での配信と close-out"
...
iteration: null
owners: ["daishiman"]
tags: ["total-cashflow-screen", "p13", "release-deploy"]
file_path: "tasks/feat-total-cashflow-screen/sys-tcscreen-p13.md"
template_id: "task"
...
resource_scope: ["docs/total-cashflow-screen/close-out.md"]
purpose: null
goal: null
scope_in: null
scope_out: null
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-total-cashflow-screen/sys-tcscreen-p13.md", "confidence": 0.95}]
...
- parent_feature: feat-total-cashflow-screen
- phase_ref: P13
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-total-cashflow-screen/sys-tcscreen-p13.md
...
package 全体を単一 PR で default branch へ配信し、migration gate と runtime schema guard の通過、本番での総収支画面の表示確認、close-out を記録する。

## 背景

migration は本番 Worker の配備より先に適用する既存ゲート (feat-deploy-migration-gate) を通す。長い作業の間に main が動くため、PR 作成前に fetch し直して migration 番号の衝突を確認する。
...
- Entry gate: SYS-TCSCREEN-P12 が完了していること
...
- Infrastructure: applicable: build:artifact と deploy.yml の migration 適用順を確認する
- Security: N/A: 本 phase は文書の作成だけを行い、この層のコードを変更しない
- Quality: applicable: CI の全ジョブが緑であることを確認する
- Documentation: N/A: 本 phase は文書の作成だけを行い、この層のコードを変更しない
- Operations: applicable: 本番での表示確認と close-out を記録する
...
- Produced artifacts:
- docs/total-cashflow-screen/close-out.md
- Consumed artifacts:
...
- PR body contract: dev-graph graph_node_id SYS-TCSCREEN-P13 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-TCSCREEN-P13 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-TCSCREEN-P13 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-TCSCREEN-P12) が完了し、write_scope が他の active lease と重複しないこと
...
- Rollback trigger and steps: 直前のビルドへ Worker を戻す。migration は追加のみのため既存データを失わず、D1 の取り消しは不要。
