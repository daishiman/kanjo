---
graph_node_id: "SYS-HOUSEHOLD-P05"
artifact_kind: "task"
artifact_subtypes: []
title: "household-summary 純関数・3 経路 API・owner_labels・家計収支画面・旧参照除去の最終実装"
...
file_path: "tasks/feat-household-cashflow/sys-household-p05.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-household-cashflow/sys-household-p05.md", "confidence": 0.95}]
tracker_binding: "beads"
beads_linkage: {"bd_issue_id": "kanjo-fzu.5", "linked_at": "2026-09-18T13:37:43Z", "sync_state": "linked"}
...
# household-summary 純関数・3 経路 API・owner_labels・家計収支画面・旧参照除去の最終実装
...
- 空は集計対象台帳行 0 件、カテゴリ詳細は `current` / `monthTotal` / 最大 5 件プレビューを分離、振替は選択月の全件 (抜粋なし) をカード内に出し循環導線を置かない。
- 旧 `household()` / `HouseholdData` / `OWNER_LABEL` 直参照が 0 件である。
