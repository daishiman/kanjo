---
graph_node_id: "SYS-CLASSIFY-P11"
artifact_kind: "task"
artifact_subtypes: []
title: "再現可能な証跡索引の作成"
...
iteration: null
owners: ["daishiman"]
tags: ["classify-screen", "p11", "evidence"]
file_path: "tasks/feat-classify-screen/sys-classify-p11.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-classify-screen/sys-classify-p11.md", "confidence": 0.95}]
...
- parent_feature: feat-classify-screen
- phase_ref: P11
- classification: confidence 0.95、reason 単一責務の実行タスクであり、artifact_kind は task 以外に取り得ない、candidate tasks/feat-classify-screen/sys-classify-p11.md
...
受入 S1〜S6・AT-01〜AT-21 の各項目から、証跡のファイルと再現コマンドへ辿れる索引を作成する。

## 背景

他 phase の成果物と証跡を後から再現できるようにするため、索引を 1 か所にまとめる。
...
- Documentation: applicable: 索引を docs へ記録する
...
- Compatibility/migration/backfill: N/A: 索引作成のみ
...
- PR body contract: dev-graph graph_node_id SYS-CLASSIFY-P11 を本文に記載し、default branch を対象にする
- Ownership boundary: system-dev-planner は intent を宣言するのみで、dev-graph が実際の mutation/reconciliation を行う

## Branch and worktree execution

- Branch: dev-graph 登録後に C15 が devgraph/SYS-CLASSIFY-P11 として割り当てる。system-dev-planner は事前割り当てを行わない
- Worktree lease: 実装着手前に SYS-CLASSIFY-P11 の worktree lease を claim し、heartbeat/release を行う
- Parallel safety: depends_on (SYS-CLASSIFY-P10) が完了し、write_scope が他の active lease と重複しないこと
...
- Acceptance:
- 受入 S1〜S6・AT-01〜AT-21 の各項目から、証跡のファイルと再現コマンドへ辿れる。
- Automated commands:
- pnpm lint
- Required evidence:
- docs/classify-screen/design-decisions.md
